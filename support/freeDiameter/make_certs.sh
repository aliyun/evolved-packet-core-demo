...
# CA self certificate
openssl req  -new -batch -x509 -days 3650 -nodes -newkey rsa:1024 -out $1/cacert.pem -keyout cakey.pem -subj /CN=ca.localdomain/C=SH/ST=HK/L=Jiangwan/O=VEPC/OU=Wnet

#mme
openssl genrsa -out $1/mme.key.pem 1024
openssl req -new -batch -out mme.csr.pem -key $1/mme.key.pem -subj /CN=mme.localdomain/C=SH/ST=HK/L=Jiangwan/O=VEPC/OU=Wnet
...
openssl req -new -batch -out hss.csr.pem -key $1/hss.key.pem -subj /CN=hss.localdomain/C=SH/ST=HK/L=Jiangwan/O=VEPC/OU=Wnet
...
openssl req -new -batch -out pgw.csr.pem -key $1/pgw.key.pem -subj /CN=pgw.localdomain/C=SH/ST=HK/L=Jiangwan/O=VEPC/OU=Wnet
...
openssl req -new -batch -out pcrf.csr.pem -key $1/pcrf.key.pem -subj /CN=pcrf.localdomain/C=SH/ST=HK/L=Jiangwan/O=VEPC/OU=Wnet
