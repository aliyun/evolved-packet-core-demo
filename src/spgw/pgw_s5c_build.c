#define TRACE_MODULE _spgw_s5c_build
...
status_t pgw_s5c_build_create_session_response(
        pkbuf_t **pkbuf, c_uint8_t type, spgw_sess_t *sess,
        gx_message_t *gx_message, gtp_create_session_request_t *req)
{
    ...
    rv = gtp_sockaddr_to_f_teid(
        spgw_self()->gtpc_addr, spgw_self()->gtpc_addr6, &pgw_s5c_teid, &len);
    ...
    rv = gtp_sockaddr_to_f_teid(
        spgw_self()->gtpu_addr, spgw_self()->gtpu_addr6, &pgw_s5u_teid, &len);
    d_assert(rv == CORE_OK, return CORE_ERROR,);
...
static void encode_traffic_flow_template(gtp_tft_t *tft, spgw_bearer_t *bearer)
{
    int i, j, len;
    pgw_pf_t *pf = NULL;
...
status_t pgw_s5c_build_create_bearer_request(
        pkbuf_t **pkbuf, c_uint8_t type, spgw_bearer_t *bearer)
{
    status_t rv;
    spgw_sess_t *sess = NULL;
    spgw_bearer_t *linked_bearer = NULL;
        ...
        switch(ue.ids[i].id)
        {
            ...
            case PCO_ID_INTERNET_PROTOCOL_CONTROL_PROTOCOL:
            {
                if (data[0] == 1) /* Code : Configuration Request */
                {
                    ...
                    {
                        rv = core_ipsubnet(
                                &dns_primary, spgw_self()->dns[0], NULL);
                        d_assert(rv == CORE_OK, return CORE_ERROR,);
                    ...
                    {
                        rv = core_ipsubnet(
                                &dns_secondary, spgw_self()->dns[1], NULL);
                        d_assert(rv == CORE_OK, return CORE_ERROR,);
            ...
            case PCO_ID_DNS_SERVER_IPV6_ADDRESS_REQUEST:
            {
                ...
                {
                    rv = core_ipsubnet(
                            &dns6_primary, spgw_self()->dns6[0], NULL);
                    d_assert(rv == CORE_OK, return CORE_ERROR,);
                    pgw.ids[pgw.num_of_id].id = ue.ids[i].id;
                ...
                {
                    rv = core_ipsubnet(
                            &dns6_secondary, spgw_self()->dns6[1], NULL);
                    d_assert(rv == CORE_OK, return CORE_ERROR,);
                    pgw.ids[pgw.num_of_id].id = ue.ids[i].id;
                    pgw.ids[pgw.num_of_id].len = IPV6_LEN;
            ...
            {
                if (spgw_self()->num_of_p_cscf)
                {
                    rv = core_ipsubnet(&p_cscf,
                        spgw_self()->p_cscf[spgw_self()->p_cscf_index], NULL);
                    d_assert(rv == CORE_OK, return CORE_ERROR,);
                    pgw.ids[pgw.num_of_id].id = ue.ids[i].id;
                    pgw.ids[pgw.num_of_id].len = IPV4_LEN;
                    ...
                    spgw_self()->p_cscf_index++;
                    spgw_self()->p_cscf_index %= spgw_self()->num_of_p_cscf;
                }
                break;
            }
            case PCO_ID_P_CSCF_IPV6_ADDRESS_REQUEST:
            {
                if (spgw_self()->num_of_p_cscf6)
                {
                    rv = core_ipsubnet(&p_cscf6,
                        spgw_self()->p_cscf6[spgw_self()->p_cscf6_index], NULL);
                    d_assert(rv == CORE_OK, return CORE_ERROR,);
                    pgw.ids[pgw.num_of_id].id = ue.ids[i].id;
                    pgw.ids[pgw.num_of_id].len = IPV6_LEN;
                    ...
                    spgw_self()->p_cscf6_index++;
                    spgw_self()->p_cscf6_index %= spgw_self()->num_of_p_cscf6;
                }
                break;
            }
