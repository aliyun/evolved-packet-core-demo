...
static status_t bearer_binding(spgw_sess_t *sess, gx_message_t *gx_message);

void pgw_gx_handle_cca_initial_request(
        spgw_sess_t *sess, gx_message_t *gx_message,
        gtp_xact_t *xact, gtp_create_session_request_t *req)
{
    ...
    d_assert(rv == CORE_OK, return, "S5C build error");
    ...
    gtp_set_pkt_hdr(&h, pkbuf);

    event_set(&e, SGW_EVT_S5C_MESSAGE);
    event_set_param1(&e, (c_uintptr_t)pkbuf);
    event_set_param2(&e, (c_uintptr_t)xact->index);
    rv = spgw_event_send(&e);
    if (rv != CORE_OK)
    {
        d_error("spgw_event_send SGW_EVT_S5C_MESSAGE error");
        pkbuf_free(pkbuf);
        return;
    }
        ...
        gtp_xact_t *xact, gtp_delete_session_request_t *req)
{
    ...
    d_assert(rv == CORE_OK, return, "xact_commit error");
#endif
    d_error("Moan: %s %d, teid=%x, type=%d\n", __FUNCTION__, __LINE__, h.teid, h.type);
