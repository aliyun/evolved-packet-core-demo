...
void pgw_gx_send_ccr(spgw_sess_t *sess, gtp_xact_t *xact,
        pkbuf_t *gtpbuf, c_uint32_t cc_request_type)
{
    ...
    /* TODO: work run for gx_sid is not null sometimes */
    if (cc_request_type == GX_CC_REQUEST_TYPE_INITIAL_REQUEST)
    {
        if (sess->gx_sid)
        {
            if (sess->gx_sid)
                CORE_FREE(sess->gx_sid);
...
status_t pgw_fd_init(void)
{
    ...
    ret = fd_init(FD_MODE_CLIENT|FD_MODE_SERVER,
                spgw_self()->fd_conf_path, spgw_self()->fd_config);
