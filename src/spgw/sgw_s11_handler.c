#define TRACE_MODULE _spgw_s11_handler
...
static int delete_gtp_flow_for_ue(c_uint32_t *ue_addr)
{
    ...
    snprintf(cmd, 512, "/epc/bin/gtp_flow del %s-10.10.10.10:2152:1234-10.10.10.11:2152:1234 &", ip_buf);
    d_trace(1, "spgw: %s\n", cmd);
    system(cmd);
...
int update_gtp_flow(int is_add,
                    spgw_bearer_t *bearer,
                    gtp_node_t *enb,
                    c_uint32_t dl_s1u_teid,
                    c_uint32_t ul_s1u_teid)
{
    struct in_addr ue_ip, epc_ip;
    char buf[CORE_ADDRSTRLEN], ue_ip_buf[CORE_ADDRSTRLEN], epc_ip_buf[CORE_ADDRSTRLEN];
    char cmd[512] = {0};
    int local_port = -1;
    c_sockaddr_t    *gtpu_addr=NULL;
    spgw_sess_t     *sess;

    d_assert(bearer, return -1, "Null param");
    
    sess = bearer->sess;
    d_assert(sess, return -1, "Null param");
    d_assert(sess->ipv4, return -1, "Null param");

    d_assert(enb, return -1, "Null param");
    d_assert(spgw_self()->gtpu_addr, return -1, "Null param");

    local_port = gtp_get_local_port(enb);
    if (local_port == -1)
    {
        d_error("can not get local s1u gtp sock port!");
        return -1;
    }

    /*
    d_trace(1, "ue_ip:0x%x R:0x%x:%d:%x L:0x%x:%d:%d)\n",
                sess->ipv4->addr[0],
                enb->ip.both.addr,
                local_port,
                dl_s1u_teid,
                spgw_self()->gtpu_addr->sin.sin_addr.s_addr,
                local_port,
                ul_s1u_teid);*/

    ue_ip.s_addr = sess->ipv4->addr[0];
    strncpy(ue_ip_buf, inet_ntoa(ue_ip), CORE_ADDRSTRLEN);

    //epc_ip.s_addr = spgw_self()->gtpu_addr->sin.sin_addr.s_addr;
    gtpu_addr = gtp_local_addr_by_cell_id(&spgw_self()->gtpu_list, bearer->e_cgi.cell_id);
    if (gtpu_addr == NULL) {
        gtpu_addr = spgw_self()->gtpu_addr;
    }
    epc_ip.s_addr = gtpu_addr->sin.sin_addr.s_addr;
    strncpy(epc_ip_buf, inet_ntoa(epc_ip), CORE_ADDRSTRLEN);

    snprintf(cmd, 512, "/epc/bin/gtp_flow add %s-%s:%d:%d-%s:%d:%d &",
                       inet_ntoa(ue_ip),
                       CORE_ADDR(enb->sa_list, buf),
                       CORE_PORT(enb->sa_list),
                       dl_s1u_teid,
                       epc_ip_buf,
                       local_port,
                       ul_s1u_teid);
                                           ...
                                           gtp_message_t *gtp_message)
{
    ...
    gtp_uli_t uli;
    event_t e;
    c_uint8_t pdn_type=GTP_PDN_TYPE_NON_IP;
    ...
    {
        pdn_type = req->pdn_type.u8;
    }

    d_assert(sgw_ue, return, "Null param");
    sess = sgw_sess_find_by_ebi(sgw_ue, req->bearer_contexts_to_be_created.eps_bearer_id.u8);
    if (!sess)
    {
        ...
        sess = spgw_sess_add(sgw_ue, apn, pdn_type, req->bearer_contexts_to_be_created.eps_bearer_id.u8);
    ...
    rv = gtp_sockaddr_to_f_teid(
        spgw_self()->gtpc_addr, spgw_self()->gtpc_addr6, &sgw_s5c_teid, &len);
...
#if 0
    pgw = gtp_find_node(&spgw_self()->pgw_s5c_list, pgw_s5c_teid);
    if (!pgw)
    {
        pgw = gtp_add_node(&spgw_self()->pgw_s5c_list, pgw_s5c_teid,
            spgw_self()->gtpc_port,
    ...
    rv = gtp_sockaddr_to_f_teid(
        spgw_self()->gtpu_addr,  spgw_self()->gtpu_addr6, &sgw_s5u_teid, &len);
    d_assert(rv == CORE_OK, return,);
    ...
    gtp_set_pkt_hdr(&gtp_message->h, pkbuf);

    event_set(&e, PGW_EVT_S5C_MESSAGE);
    event_set_param1(&e, (c_uintptr_t)pkbuf);
    event_set_param2(&e, (c_uintptr_t)s11_xact->index);
    event_set_param3(&e, (c_uintptr_t)sess);
    ...
    sgw_ue_t *sgw_ue, gtp_modify_bearer_request_t *req)
{
    ...
    enb = gtp_find_node(&spgw_self()->enb_s1u_list, enb_s1u_teid);
    if (!enb)
    {
        enb = gtp_add_node(&spgw_self()->enb_s1u_list, enb_s1u_teid,
            spgw_self()->gtpu_port,
            context_self()->parameter.no_ipv4,
            context_self()->parameter.no_ipv6,
            context_self()->parameter.prefer_ipv4);
        ...
        rv = gtp_connect(spgw_self()->gtpu_sock, spgw_self()->gtpu_sock6, enb);
#else
        rv = gtp_client(enb);
#endif
        d_assert(rv == CORE_OK, return,);
    ...
    sgw_ue_t *sgw_ue, gtp_message_t *gtp_message)
{
    ...
    if (sess->ipv4) {
        delete_gtp_flow_for_ue(&sess->ipv4->addr[0]);
    }
...
void sgw_s11_handle_lo_dldata_notification(spgw_bearer_t *bearer)
{
    status_t rv;
        ...
        gtp_create_indirect_data_forwarding_tunnel_request_t *req)
{
    ...
    for (i = 0; req_bearers[i]->presence; i++)
    {
        ...
        if (req_bearers[i]->s1_u_enodeb_f_teid.presence)
        {
            ...
            enb = gtp_find_node(&spgw_self()->enb_s1u_list, req_teid);
            if (!enb)
            {
                enb = gtp_add_node(&spgw_self()->enb_s1u_list, req_teid,
                    spgw_self()->gtpu_port,
                    context_self()->parameter.no_ipv4,
                    context_self()->parameter.no_ipv6,
                    context_self()->parameter.prefer_ipv4);
            ...
            /* TODO: support ipv6  */
            //rv = gtp_sockaddr_to_f_teid(spgw_self()->gtpu_addr, spgw_self()->gtpu_addr6, &rsp_dl_teid[i], &len);
            gtpu_addr = gtp_local_addr_by_cell_id(&spgw_self()->gtpu_list, sess->e_cgi.cell_id);
            if (gtpu_addr == NULL) {
                gtpu_addr = spgw_self()->gtpu_addr;
            }
            rv = gtp_sockaddr_to_f_teid(gtpu_addr, spgw_self()->gtpu_addr6, &rsp_dl_teid[i], &len);
        ...
        if (req_bearers[i]->s12_rnc_f_teid.presence)
        {
            ...
            /* TODO: support ipv6  */
            //rv = gtp_sockaddr_to_f_teid(spgw_self()->gtpu_addr, spgw_self()->gtpu_addr6, &rsp_ul_teid[i], &len);
            gtpu_addr = gtp_local_addr_by_cell_id(&spgw_self()->gtpu_list, sess->e_cgi.cell_id);
            if (gtpu_addr == NULL) {
                gtpu_addr = spgw_self()->gtpu_addr;
            }
            rv = gtp_sockaddr_to_f_teid(gtpu_addr, spgw_self()->gtpu_addr6, &rsp_ul_teid[i], &len);
            d_assert(rv == CORE_OK, return,);
