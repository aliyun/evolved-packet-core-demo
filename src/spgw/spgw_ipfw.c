...
spgw_bearer_t *pgw_bearer_find_by_packet(pkbuf_t *pkt)
{
    ...
    c_uint32_t ipv4_match = 0, ipv6_match = 0;
    c_uint8_t proto = 0;
    ...
    d_trace(5, "[PGW] PROTO:%d SRC:%08x %08x %08x %08x\n", proto, ntohl(src_addr[0]), ntohl(src_addr[1]), ntohl(src_addr[2]), ntohl(src_addr[3]));
    d_trace(5, "[PGW] HLEN:%d  DST:%08x %08x %08x %08x\n", ip_hlen, ntohl(dst_addr[0]), ntohl(dst_addr[1]), ntohl(dst_addr[2]), ntohl(dst_addr[3]));
    ...
    for (hi = pgw_sess_first(); hi; hi = pgw_sess_next(hi)) {
        ...
        if ((sess->ipv6 && memcmp(dst_addr, sess->ipv6->addr, addr_len) == 0)) {
            ipv6_match = 1;
        }

        if ( ipv4_match == 1 || ipv6_match == 1 ) {
            spgw_bearer_t *default_bearer = NULL;
            ...
            /* downlink */
            if (ipv4_match == 1) {
                spgw_ue_flow_stat_update(*dst_addr, 0, pkt->len);
            }

            bearer = spgw_bearer_next(default_bearer);
            /* Find the bearer with matched */
            for (; bearer; bearer = spgw_bearer_next(bearer)) {
                pgw_pf_t *pf = NULL;
                ...
                for (pf = pgw_pf_first(bearer); pf; pf = pgw_pf_next(pf)) {
                    ...
                    if (memcmp(src_mask, pf->rule.ip.local.addr, addr_len) == 0 &&
                        memcmp(dst_mask, pf->rule.ip.remote.addr, addr_len) == 0) {
                        /* Protocol match */
                        ...
                        if (pf->rule.proto == proto) {
                            if (pf->rule.proto == IPPROTO_TCP) {
                                struct tcphdr *tcph = (struct tcphdr *)((char *)pkt->payload + ip_hlen);

                                /* Source port */
                                if (pf->rule.port.local.low && ntohs(tcph->th_sport) < pf->rule.port.local.low) {
                                    continue;
                                }

                                if (pf->rule.port.local.high && ntohs(tcph->th_sport) > pf->rule.port.local.high) {
                                    continue;
                                }

                                /* Dst Port*/
                                if (pf->rule.port.remote.low && ntohs(tcph->th_dport) < pf->rule.port.remote.low) {
                                    continue;
                                }

                                if (pf->rule.port.remote.high && ntohs(tcph->th_dport) > pf->rule.port.remote.high) {
                                    continue;
                                }
                                ...
                                struct udphdr *udph = (struct udphdr *)((char *)pkt->payload + ip_hlen);

                                /* Source port */
                                if (pf->rule.port.local.low && ntohs(udph->uh_sport) < pf->rule.port.local.low) {
                                    continue;
                                }

                                if (pf->rule.port.local.high && ntohs(udph->uh_sport) > pf->rule.port.local.high) {
                                    continue;
                                }

                                /* Dst Port*/
                                if (pf->rule.port.remote.low && ntohs(udph->uh_dport) < pf->rule.port.remote.low) {
                                    continue;
                                }

                                if (pf->rule.port.remote.high && ntohs(udph->uh_dport) > pf->rule.port.remote.high) {
                                    continue;
                                }
