...
void s1ap_handle_s1_setup_request(mme_enb_t *enb, s1ap_message_t *message)
{
    ...
    if (enb->addr)
    {
        mgmt_report_enb_online(enb->enb_id, enb->addr->sin.sin_addr);
    }
    ...
    for (i = 0; i < SupportedTAs->list.count; i++)
    {
        ...
        SupportedTAs_Item = (S1AP_SupportedTAs_Item_t *)SupportedTAs->list.array[i];
        ...
        for (j = 0; j < SupportedTAs_Item->broadcastPLMNs.list.count; j++)
        {
            ...
            pLMNidentity = (S1AP_PLMNidentity_t *)SupportedTAs_Item->broadcastPLMNs.list.array[j];
            d_assert(pLMNidentity, return,);

            memcpy(&enb->supported_ta_list[enb->num_of_supported_ta_list].tac, tAC->buf, sizeof(c_uint16_t));
            enb->supported_ta_list[enb->num_of_supported_ta_list].tac = ntohs(enb->supported_ta_list[enb->num_of_supported_ta_list].tac);
            memcpy(&enb->supported_ta_list[enb->num_of_supported_ta_list].plmn_id, pLMNidentity->buf, sizeof(plmn_id_t));
            d_trace(5, "    PLMN_ID[MCC:%d MNC:%d] TAC[%d]\n",
                plmn_id_mcc(&enb->supported_ta_list[enb->num_of_supported_ta_list].plmn_id),
                plmn_id_mnc(&enb->supported_ta_list[enb->num_of_supported_ta_list].plmn_id),
    ...
    {
        if (mme_is_enb_s1c_ip_valid(enb->addr) == CORE_ERROR)
        {
            d_trace(3, "[MME] S1-Setup failure\n");
            d_assert(s1ap_build_setup_failure(&s1apbuf, S1AP_Cause_PR_misc, S1AP_CauseMisc_unspecified, S1AP_TimeToWait_v10s) == CORE_OK, return, "s1ap_build_setup_failure() failed");
        }
        else
        {
            d_trace(1, "### [S1AP] Enb Online [%s] [%d]\n", CORE_ADDR(enb->addr, buf), enb_id);
            d_assert(s1ap_build_setup_rsp(&s1apbuf) == CORE_OK, return, "s1ap_build_setup_rsp() failed");
        }
    }
    else
    {
        d_trace(3, "[MME] S1-Setup failure\n");
        d_assert(s1ap_build_setup_failure(&s1apbuf, group, cause, S1AP_TimeToWait_v10s) == CORE_OK, return, "s1ap_build_setup_failure() failed");
    }
...
void s1ap_handle_initial_ue_message(mme_enb_t *enb, s1ap_message_t *message)
{
    ...
    if (!enb_ue)
    {
        ...
        if (S_TMSI)
        {
            ...
            if (!mme_ue)
            {
                d_warn("Unknown UE by S_TMSI[G:%d,C:%d,M_TMSI:0x%x]", guti.mme_gid, guti.mme_code, guti.m_tmsi);
            }
            else
            {
                ...
                if (ECM_CONNECTED(mme_ue))
                {
                   ...
                   /* Implcit S1 release */
                    d_trace(5, "Implicit S1 release(ecm1)\n");
                ...
                /* moan add: for nsa mode jingxin eNB&gNB, UE alaways stay in 4G issue */
    ...
    memcpy(&enb_ue->nas.tai.plmn_id, pLMNidentity->buf, sizeof(enb_ue->nas.tai.plmn_id));
    ...
    memcpy(&enb_ue->nas.e_cgi.plmn_id, pLMNidentity->buf, sizeof(enb_ue->nas.e_cgi.plmn_id));
    memcpy(&enb_ue->nas.e_cgi.cell_id, cell_ID->buf, sizeof(enb_ue->nas.e_cgi.cell_id));
    ...
    d_assert(s1ap_send_to_nas(enb_ue, S1AP_ProcedureCode_id_initialUEMessage, NAS_PDU) == CORE_OK, , "s1ap_send_to_nas failed");
}
...
void s1ap_handle_ue_capability_info_indication(mme_enb_t *enb, s1ap_message_t *message)
{
    char buf[CORE_ADDRSTRLEN];
    int i;
        ...
        mme_enb_t *enb, s1ap_message_t *message)
{
    ...
    send_ue_online_to_mgmt_agent(mme_ue);
        ...
        mme_enb_t *enb, s1ap_message_t *message)
{
    ...
    if (mme_ue)
    {
        if (Cause->present == S1AP_Cause_PR_radioNetwork) {
            send_ue_offline_to_mgmt_agent(mme_ue);
        }
...
void s1ap_handle_paging(mme_ue_t *mme_ue)
{
    ...
    for (hi = mme_enb_first(); hi; hi = mme_enb_next(hi))
    {
        ...
        for (i = 0; i < enb->num_of_supported_ta_list; i++)
        {
            if (!memcmp(&enb->supported_ta_list[i], &mme_ue->tai, sizeof(tai_t)))
            {
                if (ECM_IDLE(mme_ue) || (mme_ue->enb_ue && mme_ue->enb_ue->ue_ctx_rel_action == S1AP_UE_CTX_REL_S1_NORMAL_RELEASE))
                {
                    ...
                    /* Send to enb */
                    d_assert(s1ap_send_to_enb(enb, s1apbuf, S1AP_NON_UE_SIGNALLING) == CORE_OK, return, "s1ap send error");
                }
            }
        }
    }
...
void s1ap_handle_path_switch_request(mme_enb_t *enb, s1ap_message_t *message)
{
    status_t rv;
    char buf[CORE_ADDRSTRLEN];
    int i;
...
void s1ap_handle_e_rab_modification_indication(mme_enb_t *enb, s1ap_message_t *message)
{
    status_t rv;
    char buf[CORE_ADDRSTRLEN];
    int i;
    ...
    d_trace(3, "[MME] E_RAB modification indication\n");
    ...
    if (!enb_ue)
    {
        d_error("Cannot find UE from MME-UE-S1AP-ID[%d] and eNB[%s:%d]",
                *MME_UE_S1AP_ID, CORE_ADDR(enb->addr, buf), enb->enb_id);
    ...
    {
        //S1AP_E_RABToBeSwitchedDLItemIEs_t *ie2 = NULL;
        S1AP_E_RABToBeModifiedItemBearerModIndIEs_t *ie2 = NULL;
        //S1AP_E_RABToBeSwitchedDLItem_t *e_rab = NULL;
        S1AP_E_RABToBeModifiedItemBearerModInd_t *e_rab = NULL;

        mme_bearer_t *bearer = NULL;

        //ie2 = (S1AP_E_RABToBeSwitchedDLItemIEs_t *)E_RABToBeSwitchedDLList->list.array[i];
        ie2 = (S1AP_E_RABToBeModifiedItemBearerModIndIEs_t *)E_RABToBeModifiedListBearerModInd->list.array[i];
        d_assert(ie2, return,);

        //e_rab = &ie2->value.choice.E_RABToBeSwitchedDLItem;
        e_rab = &ie2->value.choice.E_RABToBeModifiedItemBearerModInd;
        d_assert(e_rab, return, "Null param");

        bearer = mme_bearer_find_by_ue_ebi(mme_ue, e_rab->e_RAB_ID);
        d_assert(bearer, return, "Cannot find e_RAB_ID[%d]", e_rab->e_RAB_ID);

        memcpy(&bearer->enb_s1u_teid, e_rab->dL_GTP_TEID.buf, sizeof(bearer->enb_s1u_teid));
        bearer->enb_s1u_teid = ntohl(bearer->enb_s1u_teid);
        rv = s1ap_BIT_STRING_to_ip(&e_rab->transportLayerAddress, &bearer->enb_s1u_ip);
        d_assert(rv == CORE_OK, return,);

        GTP_COUNTER_INCREMENT(mme_ue, GTP_COUNTER_MODIFY_BEARER_BY_E_RAB_MODIFY);
...
void s1ap_handle_handover_failure(mme_enb_t *enb, s1ap_message_t *message)
{
    ...
    //mme_restore_info_when_ho_failure(source_ue, target_ue);
    rv = source_ue_deassociate_target_ue(target_ue->source_ue);
    d_assert(rv == CORE_OK, return,"source_ue_deassociate_target_ue failed");
}
...
void s1ap_handle_handover_notification(mme_enb_t *enb, s1ap_message_t *message)
{
    ...
    //rv = mme_remove_source_ue_when_ho_success(source_ue, target_ue);
    //d_assert(rv == CORE_OK,,"mme_remove_source_ue_when_ho_success() failed");
