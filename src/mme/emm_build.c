...
//#define NSA_R_DCNR  1     // disable NSA
#define NSA_R_DCNR  0       // enbale NSA

status_t emm_build_attach_accept(pkbuf_t **emmbuf, mme_ue_t *mme_ue, pkbuf_t *esmbuf)
{
    status_t rv;
    nas_message_t message;
    nas_attach_accept_t *attach_accept = &message.emm.attach_accept;
    nas_eps_attach_result_t *eps_attach_result = &attach_accept->eps_attach_result;
    nas_gprs_timer_t *t3412_value = &attach_accept->t3412_value;
    int served_tai_index = 0;
    nas_eps_mobile_identity_t *guti = &attach_accept->guti;
    nas_eps_network_feature_support_t *eps_network_feature_support = &attach_accept->eps_network_feature_support;
    ...
    d_assert(served_tai_index >= 0 && served_tai_index < MAX_NUM_OF_SERVED_TAI, return CORE_ERROR, "Cannot find Served TAI. Check 'mme.tai' configuration");
    ...
    eps_network_feature_support->ims_vops = 0;
    eps_network_feature_support->r_dcnr = NSA_R_DCNR;

    rv = nas_security_encode(emmbuf, mme_ue, &message);
    d_assert(rv == CORE_OK && *emmbuf,, "nas_security_encode error");
    pkbuf_free(esmbuf);

    mme_inc_cnt_nas_out(CNT_NAS_OUT_AttachAccept);

    d_trace(1, "### [MM] Attach Accept [%s]\n", mme_ue->imsi_bcd);

    return CORE_OK;
}

status_t emm_build_attach_reject(pkbuf_t **emmbuf, nas_emm_cause_t emm_cause, pkbuf_t *esmbuf)
{
    status_t rv;
    nas_message_t message;
    ...
    mme_inc_cnt_nas_out(CNT_NAS_OUT_AttachReject);

    return rv;
}

status_t emm_build_identity_request(pkbuf_t **emmbuf, mme_ue_t *mme_ue)
{
    status_t rv;
    nas_message_t message;
    ...
    mme_inc_cnt_nas_out(CNT_NAS_OUT_IdentityRequest);

    return rv;
}

status_t emm_build_authentication_request(pkbuf_t **emmbuf, e_utran_vector_t *e_utran_vector)
{
    status_t rv;
    nas_message_t message;
    ...
    memcpy(authentication_request->authentication_parameter_rand.rand, e_utran_vector->rand, RAND_LEN);
    memcpy(authentication_request->authentication_parameter_autn.autn, e_utran_vector->autn, AUTN_LEN);
    authentication_request->authentication_parameter_autn.length = AUTN_LEN;

    rv = nas_plain_encode(emmbuf, &message);
    d_assert(rv == CORE_OK && *emmbuf, , "nas encode error");
    
    mme_inc_cnt_nas_out(CNT_NAS_OUT_AuthenticationRequest);
...
status_t emm_build_authentication_reject(pkbuf_t **emmbuf)
{
    ...
    mme_inc_cnt_nas_out(CNT_NAS_OUT_AuthenticationReject);

    return rv;
}

status_t emm_build_security_mode_command(pkbuf_t **emmbuf, mme_ue_t *mme_ue)
{
    status_t rv;
    int i;
    ...
    nas_security_algorithms_t *selected_nas_security_algorithms = &security_mode_command->selected_nas_security_algorithms;
    nas_key_set_identifier_t *nas_key_set_identifier = &security_mode_command->nas_key_set_identifier;
    nas_ue_security_capability_t *replayed_ue_security_capabilities = &security_mode_command->replayed_ue_security_capabilities;
    ...
    memset(&message, 0, sizeof(message));
    message.h.security_header_type = NAS_SECURITY_HEADER_INTEGRITY_PROTECTED_AND_NEW_SECURITY_CONTEXT;
    message.h.protocol_discriminator = NAS_PROTOCOL_DISCRIMINATOR_EMM;
    ...
    {
        if (mme_ue->ue_network_capability.eia & (0x80 >> mme_self()->integrity_order[i]))
        {
    ...
    {
        if (mme_ue->ue_network_capability.eea & (0x80 >> mme_self()->ciphering_order[i]))
        {
    ...
    selected_nas_security_algorithms->type_of_integrity_protection_algorithm = mme_ue->selected_int_algorithm;
    selected_nas_security_algorithms->type_of_ciphering_algorithm = mme_ue->selected_enc_algorithm;
    ...
    replayed_ue_security_capabilities->uia = mme_ue->ue_network_capability.uia & 0x7f;
    replayed_ue_security_capabilities->gea = (mme_ue->ms_network_capability.gea1 << 6) | mme_ue->ms_network_capability.extended_gea;

    replayed_ue_security_capabilities->length = sizeof(replayed_ue_security_capabilities->eea) + sizeof(replayed_ue_security_capabilities->eia);
    if (replayed_ue_security_capabilities->uea || replayed_ue_security_capabilities->uia)
        replayed_ue_security_capabilities->length = sizeof(replayed_ue_security_capabilities->eea) +
    ...
    mme_kdf_nas(MME_KDF_NAS_INT_ALG, mme_ue->selected_int_algorithm, mme_ue->kasme, mme_ue->knas_int);
    mme_kdf_nas(MME_KDF_NAS_ENC_ALG, mme_ue->selected_enc_algorithm, mme_ue->kasme, mme_ue->knas_enc);

    rv = nas_security_encode(emmbuf, mme_ue, &message);
    d_assert(rv == CORE_OK && *emmbuf, return CORE_ERROR, "emm build error");
    
    mme_inc_cnt_nas_out(CNT_NAS_OUT_SecurityModeCommand);
...
status_t emm_build_detach_accept(pkbuf_t **emmbuf, mme_ue_t *mme_ue)
{
    ...
    mme_inc_cnt_nas_out(CNT_NAS_OUT_DetachAccept);
...
{
    nas_message_t message;
    nas_tracking_area_update_accept_t *tau_accept = &message.emm.tracking_area_update_accept;
    int served_tai_index = 0;
    ...
    /* Set T3412 */
    tau_accept->presencemask |= NAS_TRACKING_AREA_UPDATE_ACCEPT_T3412_VALUE_PRESENT ;
    tau_accept->t3412_value.unit = NAS_GRPS_TIMER_UNIT_MULTIPLES_OF_DECI_HH;
    tau_accept->t3412_value.value = 9;

    /* Set TAI */
    tau_accept->presencemask |= NAS_TRACKING_AREA_UPDATE_ACCEPT_TAI_LIST_PRESENT;
    ...
    d_trace(5, "    SERVED_TAI_INDEX[%d]\n", served_tai_index);
    d_assert(served_tai_index >= 0 && served_tai_index < MAX_NUM_OF_SERVED_TAI, return CORE_ERROR, "Cannot find Served TAI. Check 'mme.tai' configuration");
    nas_tai_list_build(&tau_accept->tai_list, &mme_self()->served_tai[served_tai_index].list0, &mme_self()->served_tai[served_tai_index].list2);
    d_trace(1, "### [MM] TAU Accept [%s]\n", mme_ue->imsi_bcd);

    /* Set EPS bearer context status */
    tau_accept->presencemask |= NAS_TRACKING_AREA_UPDATE_ACCEPT_EPS_BEARER_CONTEXT_STATUS_PRESENT;
    ...
    /* Set T3423 */
    tau_accept->presencemask |= NAS_TRACKING_AREA_UPDATE_ACCEPT_T3423_VALUE_PRESENT;
    ...
    tau_accept->presencemask |= NAS_TRACKING_AREA_UPDATE_ACCEPT_EPS_NETWORK_FEATURE_SUPPORT_PRESENT;
    tau_accept->eps_network_feature_support.length = 2;
    tau_accept->eps_network_feature_support.ims_vops = 0;
    tau_accept->eps_network_feature_support.r_dcnr = NSA_R_DCNR;

    d_assert(nas_security_encode(emmbuf, mme_ue, &message) == CORE_OK && *emmbuf,,);
    
    mme_inc_cnt_nas_out(CNT_NAS_OUT_TauAccept);

    return CORE_OK;
}

status_t emm_build_tau_reject(pkbuf_t **emmbuf, nas_emm_cause_t emm_cause, mme_ue_t *mme_ue)
{
    nas_message_t message;
    nas_tracking_area_update_reject_t *tau_reject = &message.emm.tracking_area_update_reject;

    d_assert(mme_ue, return CORE_ERROR,);

    d_trace(1, "### [MM] TAU Reject [%s] [%d]\n", mme_ue->imsi_bcd, emm_cause);
    ...
    mme_inc_cnt_nas_out(CNT_NAS_OUT_TauReject);

    return CORE_OK;
}

status_t emm_build_service_reject(pkbuf_t **emmbuf, nas_emm_cause_t emm_cause, mme_ue_t *mme_ue)
{
    nas_message_t message;
    ...
    d_trace(1, "### [MM] Servcie Reject [%s] [%d]\n", mme_ue->imsi_bcd, emm_cause);
    ...
    mme_inc_cnt_nas_out(CNT_NAS_OUT_ServiceReject);
