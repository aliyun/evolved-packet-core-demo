    ...
    nas_emm_cause_t emm_cause, nas_esm_cause_t esm_cause)
{
    ...
    d_trace(1, "### [MM] Attach Reject [%s] [%d]\n", mme_ue->imsi_bcd, emm_cause);
    ...
    d_assert(rv == CORE_OK && emmbuf, esmbuf ? pkbuf_free(esmbuf) : 1; return CORE_ERROR, "emm build error");
    rv = nas_send_to_downlink_nas_transport(mme_ue, emmbuf);
    d_assert(rv == CORE_OK, esmbuf ? pkbuf_free(esmbuf) : 1; return CORE_ERROR, "nas send error");
