        ...
        gtp_xact_t *xact, mme_ue_t *mme_ue, gtp_modify_bearer_response_t *rsp)
{
    ...
    GTP_COUNTER_CHECK(mme_ue, GTP_COUNTER_MODIFY_BEARER_BY_E_RAB_MODIFY,
        rv = s1ap_send_e_rab_modification_confirm(mme_ue);
        d_assert(rv == CORE_OK,, "s1ap send error");
    );
    ...
    GTP_COUNTER_CHECK(mme_ue, GTP_COUNTER_MODIFY_BEARER_BY_HANDOVER_NOTIFY,
        ...
        send_ue_location_update_to_mgmt_agent(mme_ue);
