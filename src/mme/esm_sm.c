        ...
        case MME_EVT_ESM_MESSAGE:
        {
            ...
            switch(message->esm.h.message_type)
            {
                case NAS_PDN_CONNECTIVITY_REQUEST:
                {
                    ...
                    mme_inc_cnt_nas_in(CNT_NAS_IN_PdnConnectivityRequest);
                    rv = esm_handle_pdn_connectivity_request(bearer, &message->esm.pdn_connectivity_request);
                    if (rv != CORE_OK)
                    {
                ...
                case NAS_ESM_INFORMATION_RESPONSE:
                {
                    ...
                    d_trace(5, "    IMSI[%s] PTI[%d] EBI[%d]\n", mme_ue->imsi_bcd, sess->pti, bearer->ebi);
                    mme_inc_cnt_nas_in(CNT_NAS_IN_InformationResponse);
                    rv = esm_handle_information_response(sess, &message->esm.esm_information_response);
                    if (rv != CORE_OK)
                    {
                        FSM_TRAN(s, esm_state_exception);
                        break;
                ...
                {
                    d_trace(3, "[ESM] Activate default EPS bearer context accept\n");
                    d_trace(5, "    IMSI[%s] PTI[%d] EBI[%d]\n", mme_ue->imsi_bcd, sess->pti, bearer->ebi);

                    mme_inc_cnt_nas_in(CNT_NAS_IN_ActivateDefaultEpsBearerContextAccept);
                ...
                {
                    d_trace(3, "[ESM] Activate dedicated EPS bearer context accept\n");
                    d_trace(5, "    IMSI[%s] PTI[%d] EBI[%d]\n", mme_ue->imsi_bcd, sess->pti, bearer->ebi);
                    
                    mme_inc_cnt_nas_in(CNT_NAS_IN_ActivateDedicatedEpsBearerContextAccept);
                    /* Check if Initial Context Setup Response or 
                     *          E-RAB Setup Response is received */
                    if (MME_HAVE_ENB_S1U_PATH(bearer))
                    {
        ...
        case MME_EVT_ESM_MESSAGE:
        {
            ...
            switch(message->esm.h.message_type)
            {
                ...
                case NAS_PDN_DISCONNECT_REQUEST:
                {
                    ...
                    d_trace(5, "    IMSI[%s] PTI[%d] EBI[%d]\n", mme_ue->imsi_bcd, sess->pti, bearer->ebi);
                    mme_inc_cnt_nas_in(CNT_NAS_IN_PdnDisconnectRequest);
                ...
                case NAS_MODIFY_EPS_BEARER_CONTEXT_ACCEPT:
                {
                    ...
                    d_trace(5, "    IMSI[%s] PTI[%d] EBI[%d]\n", mme_ue->imsi_bcd, sess->pti, bearer->ebi);
                    mme_inc_cnt_nas_in(CNT_NAS_IN_ModifyEpsBearerContextAccept);
                ...
                {
                    d_trace(3, "[ESM] [A] Deactivate EPS bearer context accept\n");
                    d_trace(5, "    IMSI[%s] PTI[%d] EBI[%d]\n", mme_ue->imsi_bcd, sess->pti, bearer->ebi);
                    mme_inc_cnt_nas_in(CNT_NAS_IN_DeactivateEpsBearerContextAccept);
                    rv = mme_gtp_send_delete_bearer_response(bearer);
                    d_assert(rv == CORE_OK, return, "mme_gtp_send_delete_session_request error");
    ...
    switch (event_get(e))
    {
        ...
        case MME_EVT_ESM_MESSAGE:
        {
            ...
            switch(message->esm.h.message_type)
            {
                case NAS_DEACTIVATE_EPS_BEARER_CONTEXT_ACCEPT:
                {
                    d_trace(3, "[ESM] [D] Deactivate EPS bearer context accept\n");
                    d_trace(5, "    IMSI[%s] PTI[%d] EBI[%d]\n", mme_ue->imsi_bcd, sess->pti, bearer->ebi);
                    mme_inc_cnt_nas_in(CNT_NAS_IN_DeactivateEpsBearerContextAccept);
                ...
                default:
                {
                    d_error("Unknown message(type:%d)", message->esm.h.message_type);
                    break;
                }
            }
            break;
