...
static void common_register_state(fsm_t *s, event_t *e)
{
    ...
    switch (event_get(e))
    {
        ...
        case MME_EVT_EMM_MESSAGE:
        {
            ...
            if (message->emm.h.security_header_type == NAS_SECURITY_HEADER_FOR_SERVICE_REQUEST_MESSAGE)
            {
                d_trace(3, "[EMM] Service request\n");
                
                mme_inc_cnt_nas_in(CNT_NAS_IN_ServiceRequest);
                
                rv = emm_handle_service_request(mme_ue, &message->emm.service_request);
                if (rv != CORE_OK)
                {
                ...
                if (!MME_UE_HAVE_IMSI(mme_ue))
                {
                    ...
                    rv = nas_send_service_reject(mme_ue, EMM_CAUSE_UE_IDENTITY_CANNOT_BE_DERIVED_BY_THE_NETWORK);
                    d_assert(rv == CORE_OK,, "nas_send_service_reject() failed");
                    FSM_TRAN(s, &emm_state_exception);
                    return;
                }
                ...
                if (!SECURITY_CONTEXT_IS_VALID(mme_ue))
                {
                    ...
                    d_assert(rv == CORE_OK,, "s1ap_send_ue_context_release_command() failed");
                    return;
                }

                rv = s1ap_send_initial_context_setup_request(mme_ue);
                d_assert(rv == CORE_OK, return, "s1ap_send_initial_context_setup_request() failed");
                
                d_trace(1, "### [MM] Service Accept [%s]\n", mme_ue->imsi_bcd);
                return;
            }
            ...
            switch(message->emm.h.message_type)
            {
                case NAS_IDENTITY_RESPONSE:
                {
                    ...
                    mme_inc_cnt_nas_in(CNT_NAS_IN_IdentityResponse);
                    rv = emm_handle_identity_response(mme_ue, &message->emm.identity_response);
                    if (rv != CORE_OK)
                    {
                ...
                case NAS_ATTACH_REQUEST:
                {
                    ...
                    mme_inc_cnt_nas_in(CNT_NAS_IN_AttachRequest);
                    rv = emm_handle_attach_request(mme_ue, &message->emm.attach_request);
                    if (rv != CORE_OK)
                    {
                ...
                case NAS_TRACKING_AREA_UPDATE_REQUEST:
                {
                    ...
                    mme_inc_cnt_nas_in(CNT_NAS_IN_TauRequest);
                ...
                case NAS_TRACKING_AREA_UPDATE_COMPLETE:
                {
                    ...
                    d_trace(5, "    IMSI[%s]\n", mme_ue->imsi_bcd);
                    mme_inc_cnt_nas_in(CNT_NAS_IN_TauComplete);
                    return;
                }
                case NAS_EMM_STATUS:
                {
                    ...
                    mme_inc_cnt_nas_in(CNT_NAS_IN_EmmStatus);
                    FSM_TRAN(s, &emm_state_exception);
                    return;
                }
                case NAS_DETACH_REQUEST:
                {
                    ...
                    mme_inc_cnt_nas_in(CNT_NAS_IN_DetachRequest);
                    send_ue_offline_to_mgmt_agent(mme_ue);

                    rv = emm_handle_detach_request(mme_ue, &message->emm.detach_request_from_ue);
                    if (rv != CORE_OK)
                    {
                        d_error("emm_handle_attach_request() failed");
                        FSM_TRAN(s, emm_state_exception);
                    ...
                    d_assert(rv == CORE_OK,,"mme_send_delete_session_or_detach() failed");
        ...
        case MME_EVT_EMM_MESSAGE:
        {
            ...
            switch(message->emm.h.message_type)
            {
                case NAS_AUTHENTICATION_RESPONSE:
                {
                    ...
                    nas_authentication_response_parameter_t *authentication_response_parameter = &authentication_response->authentication_response_parameter;

                    d_trace(3, "[EMM] Authentication response\n");
                    d_trace(5, "    IMSI[%s]\n", mme_ue->imsi_bcd);

                    mme_inc_cnt_nas_in(CNT_NAS_IN_AuthenticationResponse);
                ...
                case NAS_AUTHENTICATION_FAILURE:
                {
                    ...
                    nas_authentication_failure_parameter_t *authentication_failure_parameter = &authentication_failure->authentication_failure_parameter;

                    d_trace(3, "[EMM] Authentication failure\n");
                    d_trace(5, "    IMSI[%s] EMM_CAUSE[%d]\n", mme_ue->imsi_bcd, authentication_failure->emm_cause);
                    mme_inc_cnt_nas_in(CNT_NAS_IN_AuthenticationFailure);
                    ...
                    switch(authentication_failure->emm_cause)
                    {
                        ...
                        case EMM_CAUSE_SYNCH_FAILURE:
                            ...
                            return;
                        default:
                            d_error("Unknown EMM_CAUSE{%d] in Authentication failure", authentication_failure->emm_cause);
                            break;
                    }
        ...
        case MME_EVT_EMM_MESSAGE:
        {
            ...
            switch(message->emm.h.message_type)
            {
                case NAS_SECURITY_MODE_COMPLETE:
                {
                    ...
                    d_trace(5, "    IMSI[%s]\n", mme_ue->imsi_bcd);
                    mme_inc_cnt_nas_in(CNT_NAS_IN_SecurityModeComplete);
                ...
                case NAS_SECURITY_MODE_REJECT:
                {
                    ...
                    mme_inc_cnt_nas_in(CNT_NAS_IN_SecurityModeReject);
                    break;
                }
                case NAS_ATTACH_REQUEST:
                {
        ...
        case MME_EVT_EMM_MESSAGE:
        {
            ...
            switch(message->emm.h.message_type)
            {
                case NAS_ATTACH_COMPLETE:
                {
                    ...
                    d_trace(5, "    IMSI[%s]\n", mme_ue->imsi_bcd);
                    mme_inc_cnt_nas_in(CNT_NAS_IN_AttachComplete);
                    rv = emm_handle_attach_complete(mme_ue, &message->emm.attach_complete);
                    if (rv != CORE_OK)
                    {
