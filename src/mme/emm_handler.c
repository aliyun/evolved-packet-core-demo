...
status_t emm_handle_attach_request(mme_ue_t *mme_ue, nas_attach_request_t *attach_request)
{
    int served_tai_index = 0;

    enb_ue_t *enb_ue = NULL;
    nas_eps_attach_type_t *eps_attach_type = &attach_request->eps_attach_type;
    nas_eps_mobile_identity_t *eps_mobile_identity = &attach_request->eps_mobile_identity;
    nas_esm_message_container_t *esm_message_container = &attach_request->esm_message_container;
    ...
    /* Set EPS Attach Type */
    memcpy(&mme_ue->nas_eps.attach, eps_attach_type, sizeof(nas_eps_attach_type_t));
    ...
    {
        /* Send Attach Reject */
        d_warn("Cannot find Served TAI[PLMN_ID:0x%x,TAC:%d]", mme_ue->tai.plmn_id, mme_ue->tai.tac);
        nas_send_attach_reject(mme_ue, EMM_CAUSE_TRACKING_AREA_NOT_ALLOWED, ESM_CAUSE_PROTOCOL_ERROR_UNSPECIFIED);
        return CORE_ERROR;
    }
    ...
    {
        nas_tracking_area_identity_t *last_visited_registered_tai = &attach_request->last_visited_registered_tai;

        memcpy(&mme_ue->visited_plmn_id, &last_visited_registered_tai->plmn_id, PLMN_ID_LEN);
    }
    else
    {
    ...
    memcpy(&mme_ue->ue_network_capability, &attach_request->ue_network_capability, sizeof(attach_request->ue_network_capability));

    if (attach_request->presencemask & NAS_ATTACH_REQUEST_MS_NETWORK_CAPABILITY_PRESENT)
    {
        memcpy(&mme_ue->ms_network_capability, &attach_request->ms_network_capability, sizeof(attach_request->ms_network_capability));
    }
    ...
    switch(eps_mobile_identity->imsi.type)
    {
        case NAS_EPS_MOBILE_IDENTITY_IMSI:
        {
            ...
            nas_imsi_to_bcd(&eps_mobile_identity->imsi, eps_mobile_identity->length, imsi_bcd);
    ...
    d_trace(1, "### [MM] Attach Req [%s]\n", mme_ue->imsi_bcd);

    return CORE_OK;
}

status_t emm_handle_attach_complete(mme_ue_t *mme_ue, nas_attach_complete_t *attach_complete)
{
    status_t rv;
    pkbuf_t *emmbuf = NULL;
    ...
    nas_time_zone_and_time_t *universal_time_and_local_time_zone = &emm_information->universal_time_and_local_time_zone;
    nas_daylight_saving_time_t *network_daylight_saving_time = &emm_information->network_daylight_saving_time;
    ...
    universal_time_and_local_time_zone->year = NAS_TIME_TO_BCD(xt_gmt.tm_year % 100);
    universal_time_and_local_time_zone->mon = NAS_TIME_TO_BCD(xt_gmt.tm_mon+1);
    universal_time_and_local_time_zone->mday = NAS_TIME_TO_BCD(xt_gmt.tm_mday);
    universal_time_and_local_time_zone->hour = NAS_TIME_TO_BCD(xt_gmt.tm_hour);
    ...
    {
        universal_time_and_local_time_zone->timezone = NAS_TIME_TO_BCD(xt_local.tm_gmtoff / 900);
    }
    else
    {
        universal_time_and_local_time_zone->timezone = NAS_TIME_TO_BCD((-xt_local.tm_gmtoff) / 900);
    ...
    d_trace(1, "### [MM] Attach Complete [%s]\n", mme_ue->imsi_bcd);

    return CORE_OK;
}

status_t emm_handle_identity_response(mme_ue_t *mme_ue, nas_identity_response_t *identity_response)
{
...
status_t emm_handle_detach_request(mme_ue_t *mme_ue, nas_detach_request_from_ue_t *detach_request)
{
    ...
    switch (detach_request->detach_type.detach_type)
    {
        ...
        case 7: /* 1 1 1 : reserved */
            d_warn("Unknown Detach type[%d]", detach_request->detach_type.detach_type);
            break;
    ...
    d_trace(1, "### [MM] Detach Req [%s] [1] [%d]\n", mme_ue->imsi_bcd, detach_request->detach_type.switch_off);

    return CORE_OK;
}

status_t emm_handle_service_request(mme_ue_t *mme_ue, nas_service_request_t *service_request)
{
    nas_ksi_and_sequence_number_t *ksi_and_sequence_number = &service_request->ksi_and_sequence_number;
    ...
    d_trace(1, "### [MM] Service Req [%s]\n", mme_ue->imsi_bcd);

    return CORE_OK;
}

status_t emm_handle_tau_request(mme_ue_t *mme_ue, nas_tracking_area_update_request_t *tau_request)
{
    int served_tai_index = 0;
    ...
    /* Set EPS Update Type */
    memcpy(&mme_ue->nas_eps.update, eps_update_type, sizeof(nas_eps_update_type_t));
    ...
    d_trace(1, "### [MM] TAU [%s] [0x%x] [%d]\n", mme_ue->imsi_bcd, enb_ue->nas.tai.plmn_id, enb_ue->nas.tai.tac);
    ...
    /* Store UE specific information */
    if (tau_request->presencemask & NAS_TRACKING_AREA_UPDATE_REQUEST_LAST_VISITED_REGISTERED_TAI_PRESENT)
    {
    ...
    if (tau_request->presencemask & NAS_TRACKING_AREA_UPDATE_REQUEST_UE_NETWORK_CAPABILITY_PRESENT)
    {
        memcpy(&mme_ue->ue_network_capability, &tau_request->ue_network_capability, sizeof(tau_request->ue_network_capability));
    }

    if (tau_request->presencemask & NAS_TRACKING_AREA_UPDATE_REQUEST_MS_NETWORK_CAPABILITY_PRESENT)
    {
        memcpy(&mme_ue->ms_network_capability, &tau_request->ms_network_capability, sizeof(tau_request->ms_network_capability));
    }
