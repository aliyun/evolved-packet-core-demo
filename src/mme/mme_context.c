...
s1u_bind_by_cell_id_t s1u_bind_by_cell_id_array[MAX_NUM_OF_BIND_CELL_S1U_IP];
...
status_t mme_context_init()
{
    ...
    memset(&s1u_bind_by_cell_id_array, 0, sizeof(s1u_bind_by_cell_id_array));

    /* Initialize MME context */
    memset(&self, 0, sizeof(mme_context_t));
    
    self.mme_cnt_ctx = mme_cnt_memory_init();
    d_assert(self.mme_cnt_ctx, return CORE_ERROR, "mme_cnt_memory_init failed!");
...
{
    d_assert(context_initialized == 1, return CORE_ERROR, "MME context already has been finalized");
    ...
    {
        d_error("%d not freed in M-TMSI pool[%d] in MME-Context", pool_used(&self.m_tmsi), pool_size(&self.m_tmsi));
    }
...
static status_t mme_context_validation()
{
    ...
    if (list_first(&self.s1ap_list) == NULL && list_first(&self.s1ap_list6) == NULL)
    {
    ...
    if (self.served_tai[0].list0.tai[0].num == 0 && self.served_tai[0].list2.num == 0)
    {
    ...
    {
        d_error("No mme.security.integrity_order in '%s'", context_self()->config.path);
        return CORE_ERROR;
    }
    if (self.num_of_ciphering_order == 0)
    {
        d_error("no mme.security.ciphering_order in '%s'", context_self()->config.path);
        return CORE_ERROR;
    }

    if (check_soft_serial_and_key(self.s1_intf, self.serial, self.key) != 1)
    {
        d_error("serial[%s] or key[%s] is wrong!", self.serial, self.key);
        //return CORE_ERROR;
    }
...
status_t mme_context_parse_config()
{
    ...
    while(yaml_iter_next(&root_iter))
    {
        ...
        if (!strcmp(root_key, "mme"))
        {
            ...
            while(yaml_iter_next(&mme_iter))
            {
                ...
                {
                    yaml_node_t *node = yaml_document_get_node(document, mme_iter.pair->value);
                    ...
                    else if (node->type == YAML_MAPPING_NODE)
                    {
                        ...
                        while(yaml_iter_next(&fd_iter))
                        {
                            ...
                            {
                                self.fd_config->cnf_diamid = yaml_iter_value(&fd_iter);
                            }
                            else if (!strcmp(fd_key, "realm"))
                            {
                                self.fd_config->cnf_diamrlm = yaml_iter_value(&fd_iter);
                            }
                            ...
                            {
                                self.fd_config->cnf_flags.no_sctp = yaml_iter_bool(&fd_iter);
                            }
                            else if (!strcmp(fd_key, "listen_on"))
                            {
                                self.fd_config->cnf_addr = yaml_iter_value(&fd_iter);
                            }
                            else if (!strcmp(fd_key, "load_extension"))
                            {
                                ...
                                do
                                {
                                    ...
                                    if (yaml_iter_type(&ext_array) == YAML_MAPPING_NODE)
                                    {
                                        memcpy(&ext_iter, &ext_array, sizeof(yaml_iter_t));
                                    }
                                    else if (yaml_iter_type(&ext_array) == YAML_SEQUENCE_NODE)
                                    {
                                        if (!yaml_iter_next(&ext_array))
                                            break;
                                        yaml_iter_recurse(&ext_array, &ext_iter);
                                    }
                                    else if (yaml_iter_type(&ext_array) == YAML_SCALAR_NODE)
                                    {
                                        break;
                                    }
                                    else
                                    ...
                                    if (module)
                                    {
                                        self.fd_config->ext[self.fd_config->num_of_ext].module = module;
                                        self.fd_config->ext[self.fd_config->num_of_ext].conf = conf;
                                        self.fd_config->num_of_ext++;
                                    }
                                } while(yaml_iter_type(&ext_array) == YAML_SEQUENCE_NODE);
                            }
                            else if (!strcmp(fd_key, "connect"))
                            {
                                ...
                                do
                                {
                                    ...
                                    if (yaml_iter_type(&conn_array) == YAML_MAPPING_NODE)
                                    {
                                        memcpy(&conn_iter, &conn_array, sizeof(yaml_iter_t));
                                    }
                                    ...
                                    else if (yaml_iter_type(&conn_array) == YAML_SCALAR_NODE)
                                    {
                                        break;
                                    }
                                    else
                                    ...
                                    {
                                        const char *conn_key = yaml_iter_key(&conn_iter);
                                        d_assert(conn_key, return CORE_ERROR,);
                                        ...
                                        {
                                            const char *v = yaml_iter_value(&conn_iter);
                                            if (v) port = atoi(v);
                                        }
                                        else
                                    ...
                                    if (identity && addr)
                                    {
                                        self.fd_config->conn[self.fd_config->num_of_conn].identity = identity;
                                        self.fd_config->conn[self.fd_config->num_of_conn].addr = addr;
                                        self.fd_config->conn[self.fd_config->num_of_conn].port = port;
                                        self.fd_config->num_of_conn++;
                                    }
                                } while(yaml_iter_type(&conn_array) == YAML_SEQUENCE_NODE);
                            }
                            else
                ...
                else if (!strcmp(mme_key, "serial"))
                {
                    const char *v = yaml_iter_value(&mme_iter);
                    if (v) {
                        strncpy(self.serial, v, SOFT_SERIAL_LEN);
                    }
                }
                else if (!strcmp(mme_key, "key"))
                {
                    const char *v = yaml_iter_value(&mme_iter);
                    if (v) {
                        strncpy(self.key, v, SOFT_KEY_LEN);
                    }
                }
                else if (!strcmp(mme_key, "s1_intf"))
                {
                    const char *v = yaml_iter_value(&mme_iter);
                    if (v) {
                        strncpy(self.s1_intf, v, S1_INTF_LEN);
                    }
                }
                ...
                else if (!strcmp(mme_key, "s1ap"))
                {
                    ...
                    do
                    {
                        ...
                        else if (yaml_iter_type(&s1ap_array) == YAML_SCALAR_NODE)
                        {
                            break;
                        }
                        else
                        ...
                        while(yaml_iter_next(&s1ap_iter))
                        {
                            ...
                            if (!strcmp(s1ap_key, "family"))
                            {
                                ...
                                {
                                    d_warn("Ignore family(%d) : AF_UNSPEC(%d), AF_INET(%d), AF_INET6(%d) ", family, AF_UNSPEC, AF_INET, AF_INET6);
                                    family = AF_UNSPEC;
                                }
                            }
                            else if (!strcmp(s1ap_key, "addr") || !strcmp(s1ap_key, "name"))
                            {
                                yaml_iter_t hostname_iter;
                                yaml_iter_recurse(&s1ap_iter, &hostname_iter);
                                d_assert(yaml_iter_type(&hostname_iter) != YAML_MAPPING_NODE, return CORE_ERROR,);

                                do
                                {
                                    if (yaml_iter_type(&hostname_iter) == YAML_SEQUENCE_NODE)
                                    {
                                        if (!yaml_iter_next(&hostname_iter))
                                            break;
                                    }

                                    d_assert(num <= MAX_NUM_OF_HOSTNAME, return CORE_ERROR,);
                                    hostname[num++] = yaml_iter_value(&hostname_iter);
                                } while(
                                    yaml_iter_type(&hostname_iter) == YAML_SEQUENCE_NODE);
                            }
                        ...
                        for (i = 0; i < num; i++)
                        {
                            rv = core_addaddrinfo(&list, family, hostname[i], port, 0);
                        ...
                        if (list)
                        {
                            ...
                            {
                                rv = sock_add_node(&self.s1ap_list, &node, list, AF_INET);
                                d_assert(rv == CORE_OK, return CORE_ERROR,);
                            }

                            if (context_self()->parameter.no_ipv6 == 0)
                            {
                                rv = sock_add_node(&self.s1ap_list6, &node, list, AF_INET6);
                                d_assert(rv == CORE_OK, return CORE_ERROR,);
                            }
                        ...
                        if (dev)
                        {
                            rv = sock_probe_node(context_self()->parameter.no_ipv4 ? NULL : &self.s1ap_list,
                                    context_self()->parameter.no_ipv6 ? NULL : &self.s1ap_list6,
                                    dev, self.s1ap_port);
                            d_assert(rv == CORE_OK, return CORE_ERROR,);
                        }
                ...
                else if (!strcmp(mme_key, "gtpc"))
                {
                    ...
                    do
                    {
                        ...
                        {
                            memcpy(&gtpc_iter, &gtpc_array, sizeof(yaml_iter_t));
                        }
                        else if (yaml_iter_type(&gtpc_array) == YAML_SEQUENCE_NODE)
                        {
                            if (!yaml_iter_next(&gtpc_array))
                                break;
                            yaml_iter_recurse(&gtpc_array, &gtpc_iter);
                        }
                        else if (yaml_iter_type(&gtpc_array) == YAML_SCALAR_NODE)
                        {
                            break;
                        }
                        else
                        ...
                        {
                            const char *gtpc_key = yaml_iter_key(&gtpc_iter);
                            ...
                            else if (!strcmp(gtpc_key, "addr") || !strcmp(gtpc_key, "name"))
                            {
                                yaml_iter_t hostname_iter;
                                ...
                                do
                                {
                                    ...
                                    d_assert(num <= MAX_NUM_OF_HOSTNAME, return CORE_ERROR,);
                                    hostname[num++] = yaml_iter_value(&hostname_iter);
                                } while(yaml_iter_type(&hostname_iter) == YAML_SEQUENCE_NODE);
                            }
                        ...
                        if (list)
                        {
                            if (context_self()->parameter.no_ipv4 == 0)
                            {
                                rv = sock_add_node(&self.gtpc_list, &node, list, AF_INET);
                                d_assert(rv == CORE_OK, return CORE_ERROR,);
                            }

                            if (context_self()->parameter.no_ipv6 == 0)
                            {
                                rv = sock_add_node(&self.gtpc_list6, &node, list, AF_INET6);
                                d_assert(rv == CORE_OK, return CORE_ERROR,);
                            }
                        ...
                        if (dev)
                        {
                            rv = sock_probe_node(context_self()->parameter.no_ipv4 ? NULL : &self.gtpc_list,
                                    context_self()->parameter.no_ipv6 ? NULL : &self.gtpc_list6,
                                    dev, self.gtpc_port);
                            d_assert(rv == CORE_OK, return CORE_ERROR,);
                        }
                ...
                else if (!strcmp(mme_key, "gummei"))
                {
                    ...
                    do
                    {
                        ...
                        d_assert(self.max_num_of_served_gummei <= MAX_NUM_OF_SERVED_GUMMEI, return CORE_ERROR,);
                        gummei = &self.served_gummei[self.max_num_of_served_gummei];
                        d_assert(gummei, return CORE_ERROR,);

                        if (yaml_iter_type(&gummei_array) == YAML_MAPPING_NODE)
                        {
                            memcpy(&gummei_iter, &gummei_array, sizeof(yaml_iter_t));
                        }
                        else if (yaml_iter_type(&gummei_array) == YAML_SEQUENCE_NODE)
                        {
                            if (!yaml_iter_next(&gummei_array))
                                break;
                            yaml_iter_recurse(&gummei_array, &gummei_iter);
                        }
                        else if (yaml_iter_type(&gummei_array) == YAML_SCALAR_NODE)
                        {
                            break;
                        }
                        else
                        ...
                        {
                            const char *gummei_key = yaml_iter_key(&gummei_iter);
                            ...
                            if (!strcmp(gummei_key, "plmn_id"))
                            {
                                ...
                                do
                                {
                                    ...
                                    d_assert(gummei->num_of_plmn_id <= MAX_PLMN_ID, return CORE_ERROR,);
                                    plmn_id = &gummei->plmn_id[gummei->num_of_plmn_id];
                                    d_assert(plmn_id, return CORE_ERROR,);

                                    if (yaml_iter_type(&plmn_id_array) == YAML_MAPPING_NODE)
                                    {
                                        memcpy(&plmn_id_iter, &plmn_id_array, sizeof(yaml_iter_t));
                                    }
                                    else if (yaml_iter_type(&plmn_id_array) == YAML_SEQUENCE_NODE)
                                    {
                                        if (!yaml_iter_next(&plmn_id_array))
                                            break;
                                        yaml_iter_recurse(&plmn_id_array, &plmn_id_iter);
                                    }
                                    else if (yaml_iter_type(&plmn_id_array) == YAML_SCALAR_NODE)
                                    {
                                        break;
                                    }
                                    else
                                    ...
                                    while(yaml_iter_next(&plmn_id_iter))
                                    {
                                        ...
                                        d_assert(plmn_id_key, return CORE_ERROR,);
                                    ...
                                    if (mcc && mnc)
                                    {
                                        plmn_id_build(plmn_id, atoi(mcc), atoi(mnc), strlen(mnc));
                                        gummei->num_of_plmn_id++;
                                    }

                                } while(yaml_iter_type(&plmn_id_array) == YAML_SEQUENCE_NODE);
                            }
                            else if (!strcmp(gummei_key, "mme_gid"))
                            {
                                yaml_iter_t mme_gid_iter;
                                yaml_iter_recurse(&gummei_iter, &mme_gid_iter);
                                d_assert(yaml_iter_type(&mme_gid_iter) != YAML_MAPPING_NODE, return CORE_ERROR,);
                                ...
                                do
                                {
                                    ...
                                    d_assert(gummei->num_of_mme_gid <= GRP_PER_MME, return CORE_ERROR,);
                                    mme_gid = &gummei->mme_gid[gummei->num_of_mme_gid];
                                    d_assert(mme_gid, return CORE_ERROR,);

                                    if (yaml_iter_type(&mme_gid_iter) == YAML_SEQUENCE_NODE)
                                    {
                                ...
                                } while(yaml_iter_type(&mme_gid_iter) == YAML_SEQUENCE_NODE);
                            }
                            else if (!strcmp(gummei_key, "mme_code"))
                            {
                                yaml_iter_t mme_code_iter;
                                yaml_iter_recurse(&gummei_iter, &mme_code_iter);
                                d_assert(yaml_iter_type(&mme_code_iter) != YAML_MAPPING_NODE, return CORE_ERROR,);
                                ...
                                do
                                {
                                    ...
                                    d_assert(gummei->num_of_mme_code <= CODE_PER_MME, return CORE_ERROR,);
                                    mme_code = &gummei->mme_code[gummei->num_of_mme_code];
                                    d_assert(mme_code, return CORE_ERROR,);

                                    if (yaml_iter_type(&mme_code_iter) == YAML_SEQUENCE_NODE)
                                    {
                                    ...
                                    yaml_iter_type(&mme_code_iter) == YAML_SEQUENCE_NODE);
                            }
                            else
                        ...
                        {
                            d_warn("Ignore gummei : plmn_id(%d), mme_gid(%d), mme_code(%d)", gummei->num_of_plmn_id, gummei->num_of_mme_gid, gummei->num_of_mme_code);
                            gummei->num_of_plmn_id = 0;
                            gummei->num_of_mme_gid = 0;
                            gummei->num_of_mme_code = 0;
                        }
                    } while(yaml_iter_type(&gummei_array) == YAML_SEQUENCE_NODE);
                }
                else if (!strcmp(mme_key, "tai"))
                {
                    ...
                    d_assert(self.num_of_served_tai <= MAX_NUM_OF_SERVED_TAI, return CORE_ERROR,);
                    ...
                    do
                    {
                        ...
                        else if (yaml_iter_type(&tai_array) == YAML_SEQUENCE_NODE)
                        {
                            if (!yaml_iter_next(&tai_array))
                                break;
                            yaml_iter_recurse(&tai_array, &tai_iter);
                        }
                        ...
                        while(yaml_iter_next(&tai_iter))
                        {
                            ...
                            else if (!strcmp(tai_key, "tac"))
                            {
                                ...
                                d_assert(yaml_iter_type(&tac_iter) != YAML_MAPPING_NODE, return CORE_ERROR,);

                                do
                                {
                                    const char *v = NULL;

                                    d_assert(num_of_tac <= MAX_NUM_OF_TAI, return CORE_ERROR,);
                                    if (yaml_iter_type(&tac_iter) == YAML_SEQUENCE_NODE)
                                    {
                                    ...
                                    yaml_iter_type(&tac_iter) == YAML_SEQUENCE_NODE);
                            }
                            else
                        ...
                        if (mcc && mnc && num_of_tac)
                        {
                            if (num_of_tac == 1)
                            {
                                plmn_id_build(&list2->tai[list2->num].plmn_id, atoi(mcc), atoi(mnc), strlen(mnc));
                            ...
                            {
                                int i;
                                plmn_id_build(&list0->tai[num_of_list0].plmn_id, atoi(mcc), atoi(mnc), strlen(mnc));
                        ...
                        {
                            d_warn("Ignore tai : mcc(%p), mnc(%p), num_of_tac(%d)", mcc, mnc, num_of_tac);
                        }
                ...
                else if (!strcmp(mme_key, "security"))
                {
                    ...
                    {
                        const char *security_key = yaml_iter_key(&security_iter);
                        d_assert(security_key, return CORE_ERROR,);
                        if (!strcmp(security_key, "integrity_order"))
                        {
                            yaml_iter_t integrity_order_iter;
                            yaml_iter_recurse(&security_iter, &integrity_order_iter);
                            d_assert(yaml_iter_type(&integrity_order_iter) != YAML_MAPPING_NODE, return CORE_ERROR,);

                            do
                            {
                                const char *v = NULL;

                                if (yaml_iter_type(&integrity_order_iter) == YAML_SEQUENCE_NODE)
                                {
                                ...
                                if (v) 
                                {
                                    int integrity_index = self.num_of_integrity_order;
                                    if (strcmp(v, "EIA0") == 0)
                                    {
                                        self.integrity_order[integrity_index] = NAS_SECURITY_ALGORITHMS_EIA0;
                                        self.num_of_integrity_order++;
                                    }
                                    else if (strcmp(v, "EIA1") == 0)
                                    {
                                        self.integrity_order[integrity_index] = NAS_SECURITY_ALGORITHMS_128_EIA1;
                                        self.num_of_integrity_order++;
                                    }
                                    else if (strcmp(v, "EIA2") == 0)
                                    {
                                        self.integrity_order[integrity_index] = NAS_SECURITY_ALGORITHMS_128_EIA2;
                                        self.num_of_integrity_order++;
                                    }
                                    else if (strcmp(v, "EIA3") == 0)
                                    {
                                        self.integrity_order[integrity_index] = NAS_SECURITY_ALGORITHMS_128_EIA3;
                                        self.num_of_integrity_order++;
                                    }
                                }
                            } while(yaml_iter_type(&integrity_order_iter) == YAML_SEQUENCE_NODE);
                        }
                        else if (!strcmp(security_key, "ciphering_order"))
                        {
                            yaml_iter_t ciphering_order_iter;
                            yaml_iter_recurse(&security_iter, &ciphering_order_iter);
                            d_assert(yaml_iter_type(&ciphering_order_iter) != YAML_MAPPING_NODE, return CORE_ERROR,);

                            do
                            {
                                const char *v = NULL;

                                if (yaml_iter_type(&ciphering_order_iter) == YAML_SEQUENCE_NODE)
                                {
                                ...
                                if (v) 
                                {
                                    int ciphering_index = self.num_of_ciphering_order;
                                    if (strcmp(v, "EEA0") == 0)
                                    {
                                        self.ciphering_order[ciphering_index] = NAS_SECURITY_ALGORITHMS_EEA0;
                                        self.num_of_ciphering_order++;
                                    }
                                    else if (strcmp(v, "EEA1") == 0)
                                    {
                                        self.ciphering_order[ciphering_index] = NAS_SECURITY_ALGORITHMS_128_EEA1;
                                        self.num_of_ciphering_order++;
                                    }
                                    else if (strcmp(v, "EEA2") == 0)
                                    {
                                        self.ciphering_order[ciphering_index] = NAS_SECURITY_ALGORITHMS_128_EEA2;
                                        self.num_of_ciphering_order++;
                                    }
                                    else if (strcmp(v, "EEA3") == 0)
                                    {
                                        self.ciphering_order[ciphering_index] = NAS_SECURITY_ALGORITHMS_128_EEA3;
                                        self.num_of_ciphering_order++;
                                    }
                                }
                            } while(yaml_iter_type(&ciphering_order_iter) == YAML_SEQUENCE_NODE);
                        }
                    }
                }
                else if(!strcmp(mme_key, "network_name"))
                {
                    ...
                    {
                        const char *network_name_key = yaml_iter_key(&network_name_iter);
                        d_assert(network_name_key, return CORE_ERROR,);
        ...
        else if (!strcmp(root_key, "spgw"))
        {
            ...
            while(yaml_iter_next(&mme_iter))
            {
                    ...
                    do
                    {
                        ...
                        while(yaml_iter_next(&gtpc_iter))
                        {
                            ...
                            else if (!strcmp(gtpc_key, "tac"))
                            {
                                ...
                                } while(yaml_iter_type(&tac_iter) == YAML_SEQUENCE_NODE);
                            }
                            else
                                d_warn("unknown key `%s`", gtpc_key);
                        }
                ...
                else if (!strcmp(mme_key, "gtpu"))
                {
                    yaml_iter_t gtpu_array, gtpu_iter;
                    yaml_iter_recurse(&mme_iter, &gtpu_array);
                    do
                    {
                        int family = AF_UNSPEC;
                        int num = 0;
                        const char *hostname[MAX_NUM_OF_HOSTNAME];
                        c_uint32_t cells[MAX_NUM_OF_BIND_CELL];
                        ...
                        {
                            memcpy(&gtpu_iter, &gtpu_array, sizeof(yaml_iter_t));
                        }
                        else if (yaml_iter_type(&gtpu_array) == YAML_SEQUENCE_NODE)
                        {
                            if (!yaml_iter_next(&gtpu_array))
                                break;
                            yaml_iter_recurse(&gtpu_array, &gtpu_iter);
                        }
                        else if (yaml_iter_type(&gtpu_array) == YAML_SCALAR_NODE)
                        {
                            break;
                        }
                        else
                        ...
                        {
                            const char *gtpu_key = yaml_iter_key(&gtpu_iter);
                            d_assert(gtpu_key, return CORE_ERROR,);
                            ...
                            else if (!strcmp(gtpu_key, "addr") || !strcmp(gtpu_key, "name"))
                            {
                                yaml_iter_t hostname_iter;
                            ...
                            else if (!strcmp(gtpu_key, "enb_id"))
                            {
                        ...
                        if (num == 1) {
                            rv = mme_add_s1u_ip_bind_cell_id_entry(hostname[0], &cells[0]);
                            d_assert(rv == CORE_OK, return CORE_ERROR,);
                        }
    ...
    mme_check_and_apply_dynamic_cfg();
...
status_t mme_enb_remove(mme_enb_t *enb)
{
    ...
    if (enb->addr)
    {
        mgmt_report_enb_offline(enb->enb_id, enb->addr->sin.sin_addr);
    }
...
enb_ue_t* enb_ue_add(mme_enb_t *enb)
{
    ...
    hash_set(self.mme_ue_s1ap_id_hash, &enb_ue->mme_ue_s1ap_id, sizeof(enb_ue->mme_ue_s1ap_id), enb_ue);
    list_append(&enb->enb_ue_list, enb_ue);

    /* Create S1 holding timer */
    enb_ue->holding_timer = mme_timer_create(&self.tm_service, MME_EVT_S1AP_S1_HOLDING_TIMER, self.s1_holding_timer_value * 1000);
    d_assert(enb_ue->holding_timer, return NULL, "Null param");
    timer_set_param1(enb_ue->holding_timer, enb_ue->index);

    enb_ue->source_ue = NULL;
    enb_ue->target_ue = NULL;
...
status_t enb_ue_remove(enb_ue_t *enb_ue)
{
   ...
   /* Delete All Timers */
    ...
    hash_set(self.mme_ue_s1ap_id_hash, &enb_ue->mme_ue_s1ap_id, sizeof(enb_ue->mme_ue_s1ap_id), NULL);

    /* TODO: work run solution for ho failure */
    if (enb_ue->target_ue)
    {
        rv = enb_ue_remove(enb_ue->target_ue);
        d_assert(rv == CORE_OK, , "enb_ue_remove target ue failed!");
    }
    enb_ue->target_ue = NULL;

    d_assert(enb_ue->source_ue == NULL, , "enb_ue->source_ue != NULL");
    enb_ue->source_ue = NULL;
...
mme_ue_t* mme_ue_add(enb_ue_t *enb_ue)
{
    ...
    mme_ue->t3413 = mme_timer_create(&self.tm_service, MME_EVT_EMM_T3413, self.t3413_value * 1000);
...
status_t mme_remove_source_ue_when_ho_success(enb_ue_t *source_ue, enb_ue_t *target_ue)
{
    status_t rv;
    ...
    /* De-associate S1 with NAS/EMM */
    rv = enb_ue_deassociate(source_ue);
    d_assert(rv == CORE_OK,,);

   /* Delete All Timers */
    tm_delete(source_ue->holding_timer);

    list_remove(&source_ue->enb->enb_ue_list, source_ue);

    index_free(&enb_ue_pool, source_ue);
...
void mme_restore_info_when_ho_failure(enb_ue_t *source_ue, enb_ue_t *target_ue)
{
    d_assert(source_ue, return,);
    d_assert(target_ue, return,);
    
    hash_set(self.mme_ue_s1ap_id_hash, &target_ue->mme_ue_s1ap_id, sizeof(target_ue->mme_ue_s1ap_id), NULL);
    hash_set(self.mme_ue_s1ap_id_hash, &source_ue->mme_ue_s1ap_id, sizeof(source_ue->mme_ue_s1ap_id), source_ue);
    target_ue->source_ue = NULL;
}
...
int mme_find_served_tai(tai_t *tai)
{
    ...
    for (i = 0; i < self.num_of_served_tai; i++)
    {
        ...
        {
            d_assert(list0->tai[j].type == TAI0_TYPE, return -1, "type = %d", list0->tai[j].type);
            d_assert(list0->tai[j].num < MAX_NUM_OF_TAI, return -1, "num = %d", list0->tai[j].num);

            for (k = 0; k < list0->tai[j].num; k++) 
            {
                if (memcmp(&list0->tai[j].plmn_id, &tai->plmn_id, PLMN_ID_LEN) == 0 && list0->tai[j].tac[k] == tai->tac)
                {
                    return i;
                }
            }
        ...
        if (list2->num)
        {
            d_assert(list2->type == TAI1_TYPE || list2->type == TAI2_TYPE, return -1, "type = %d", list2->type);
            d_assert(list2->num < MAX_NUM_OF_TAI, return -1, "num = %d", list2->num);

            for (j = 0; j < list2->num; j++) 
            {
                if (memcmp(&list2->tai[j].plmn_id, &tai->plmn_id, PLMN_ID_LEN) == 0 && list2->tai[j].tac == tai->tac)
                {
                    return i;
                }
            }
...
ip_t *mme_get_s1u_ip_by_cell_id(c_uint32_t cell_id)
{
    int i=0, j=0;
    s1u_bind_by_cell_id_t *node = NULL;
    
    for (i=0; i<MAX_NUM_OF_BIND_CELL_S1U_IP; i++)
    {
        node = &s1u_bind_by_cell_id_array[i];
        if (node->s1u_ip.ipv4 != 1 && node->s1u_ip.ipv6 != 1)
        {
            continue;
        }

        for(j=0; j<MAX_NUM_OF_BIND_CELL; j++)
        {
            if (cell_id == node->cells[j])
            {
                return &node->s1u_ip;
            }
        }
...
status_t mme_add_s1u_ip_bind_cell_id_entry(const char *ip, c_uint32_t *cells)
{
    c_uint32_t  ipv4;
    int         i=0, found = 0;
    ip_t        s1u_ip;
    ...
    if (strchr(ip, '.'))
    {
        ...
        d_assert(ipv4 != -1, return CORE_ERROR, );
    ...
    else if (strchr(ip, ':'))
    {
        ...
        rv = core_inet_pton(AF_INET6, ip, &addr);
        d_assert(rv == CORE_OK, return CORE_ERROR, );

        memcpy((void *)&s1u_ip.addr6, (void *)&addr.sin6.sin6_addr, IPV6_LEN);
        s1u_ip.len = IPV6_LEN;
        s1u_ip.ipv4 = 0;
        s1u_ip.ipv6 = 1;
    }
    ...
    {
        node = &s1u_bind_by_cell_id_array[i];
        if (node->s1u_ip.ipv4 == 0 && node->s1u_ip.ipv6 == 0)
        {
            found = 1;
            break;
        }
    ...
    d_assert(found == 1, return CORE_ERROR, );
    memcpy(&node->s1u_ip, &s1u_ip,  sizeof(s1u_ip));
    memcpy(&node->cells,  cells, sizeof(node->cells));

    return CORE_OK;
}

mme_cnt_ctx_t* mme_cnt_memory_init()
{
    char *addr=NULL;
	int fd;

	fd = open(MME_CNT_MEM_FILE, O_RDWR|O_CREAT, S_IROTH);
	if (fd == -1) {
        d_error("open %s failed! %s", MME_CNT_MEM_FILE, strerror(errno));
        return NULL;
    }

    if (ftruncate(fd, sizeof(mme_cnt_ctx_t)) == -1) {
        d_error("ftruncate %s failed! %s", MME_CNT_MEM_FILE, strerror(errno));
        close(fd);
        return NULL;
    }


	addr = mmap(NULL, sizeof(mme_cnt_ctx_t), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
	if (addr == MAP_FAILED) {
        d_error("mmap %s failed! %s", MME_CNT_MEM_FILE, strerror(errno));
        close(fd);
        return NULL; 
    }
    ...
    memset(addr, 0, sizeof(mme_cnt_ctx_t));

    return ((mme_cnt_ctx_t *)addr);
}

void mme_inc_cnt_s1ap_in(cnt_s1ap_in_e type)
{
    if (!self.mme_cnt_ctx) {
        return;
    }

    if (type < 0 || type >= CNT_S1AP_IN_MAX) {
        return;
    }
    self.mme_cnt_ctx->s1ap_in_cnt[type] += htonl(1);
}

void mme_inc_cnt_s1ap_out(cnt_s1ap_out_e type)
{
    if (!self.mme_cnt_ctx) {
        return;
    }

    if (type < 0 || type >= CNT_S1AP_OUT_MAX) {
        return;
    }
    self.mme_cnt_ctx->s1ap_out_cnt[type] += htonl(1);
}

void mme_inc_cnt_nas_in(cnt_nas_in_e type)
{
    if (!self.mme_cnt_ctx) {
        return;
    }

    if (type < 0 || type >= CNT_NAS_IN_MAX) {
        return;
    }
    self.mme_cnt_ctx->nas_in_cnt[type] += htonl(1);
}

void mme_inc_cnt_nas_out(cnt_nas_out_e type)
{
    if (!self.mme_cnt_ctx) {
        return;
    }

    if (type < 0 || type >= CNT_NAS_OUT_MAX) {
        return;
    }
    self.mme_cnt_ctx->nas_out_cnt[type] += htonl(1);
}

static status_t mme_context_read_file(config_t *config)
{
    FILE *file;
    ...
    d_assert(file, return CORE_ERROR, "Failed to read configuration file `%s`", config->path);
        ...
        switch (parser.error)
        {
            ...
            case YAML_READER_ERROR:
                if (parser.problem_value != -1)
                    d_error("Reader error - %s: #%X at %zd", parser.problem, parser.problem_value, parser.problem_offset);
                else
                    d_error("Reader error - %s at %zd", parser.problem, parser.problem_offset);
                break;
...
static status_t mme_context_parse_dynamic_config(config_t *config, mme_cfg_t *mme_cfg)
{
    status_t rv;
    yaml_document_t *document = NULL;
    yaml_iter_t root_iter;
    ...
    while(yaml_iter_next(&root_iter))
    {
        ...
        if (!strcmp(root_key, "mme"))
        {
            ...
            while(yaml_iter_next(&mme_iter))
            {
                const char *mme_key = yaml_iter_key(&mme_iter);
                d_assert(mme_key, return CORE_ERROR,);
                if (!strcmp(mme_key, "cfg_ver"))
                {
                    const char *v = yaml_iter_value(&mme_iter);
                    if (v)
                    {
                        strncpy(mme_cfg->cfg_ver, v, MAX_LEN_OF_VERSION);
                    }
                }
                else if (!strcmp(mme_key, "tai"))
                {
                    ...
                    d_assert(mme_cfg->num_of_served_tai <= MAX_NUM_OF_SERVED_TAI, return CORE_ERROR,);
                    list0 = &mme_cfg->served_tai[mme_cfg->num_of_served_tai].list0;
                    d_assert(list0, return CORE_ERROR,);
                    list2 = &mme_cfg->served_tai[mme_cfg->num_of_served_tai].list2;
                    d_assert(list2, return CORE_ERROR,);
                    ...
                    if (list2->num || num_of_list0)
                    {
                        mme_cfg->num_of_served_tai++;
                    }
                }
                else if (!strcmp(mme_key, "enb"))
                {
                    ip_t *enb_ip = &mme_cfg->enb_s1c_ip[0];
                    yaml_iter_t enb_iter;
                    yaml_iter_recurse(&mme_iter, &enb_iter);
                    d_assert(yaml_iter_type(&enb_iter) != YAML_MAPPING_NODE, return CORE_ERROR,);

                    do
                    {
                        const char *v = NULL;

                        if (yaml_iter_type(&enb_iter) == YAML_SEQUENCE_NODE)
                        {
                            if (!yaml_iter_next(&enb_iter))
                                break;
                        }

                        v = yaml_iter_value(&enb_iter);
                        if (v)
                        {
                            c_sockaddr_t addr;
                            ipsubnet_t ipsub;
                            ...
                            {
                                if (core_inet_pton(AF_INET, v, &addr) == CORE_OK)
                                {
                                    enb_ip->ipv4 = 1;
                                    enb_ip->ipv6 = 0;
                                    memcpy((void *)&enb_ip->addr, (void *)&addr.sin.sin_addr.s_addr, IPV4_LEN);
                                    enb_ip++;
                                }
                            }
                            else if (ipsub.family == AF_INET6)
                            {
                                if (core_inet_pton(AF_INET6, v, &addr) == CORE_OK)
                                {
                                    enb_ip->ipv4 = 0;
                                    enb_ip->ipv6 = 1;
                                    memcpy((void *)&enb_ip->addr6, (void *)&addr.sin6.sin6_addr.__in6_u.__u6_addr8, IPV6_LEN);
                                    enb_ip++;
                                }
                            }
                            else
                                d_warn("Ignore ENB : %s", v);
                        }

                    } while(yaml_iter_type(&enb_iter) == YAML_SEQUENCE_NODE);
                }
            }
        }
    }
...
static status_t mme_get_cfg(mme_cfg_t *mme_cfg)
{
    status_t rv;
    config_t config;
    c_int8_t *file_path = "/epc/cfg/vepc_dynamic.conf";
    
    d_assert(mme_cfg, return CORE_ERROR, "");

    memset(&config, 0, sizeof(config_t));

    config.path = file_path;
    rv = mme_context_read_file(&config);
    d_assert(rv == CORE_OK, return CORE_ERROR, "");

    rv = mme_context_parse_dynamic_config(&config, mme_cfg);
    d_assert(rv == CORE_OK, return CORE_ERROR, "");

    return CORE_OK;
}

status_t mme_check_and_apply_dynamic_cfg()
{
    status_t rv;
    mme_cfg_t mme_cfg;
    c_int8_t cmd[256] = {0};
    
    memset(&mme_cfg, 0, sizeof(mme_cfg_t));
    
    rv = mme_get_cfg(&mme_cfg);
    d_assert(rv == CORE_OK, return CORE_ERROR, "");

    if (strcmp(mme_cfg.cfg_ver, self.cfg_version) == 0)
    {
        /* no changed cfg */
        return CORE_OK;
    }
    ...
    self.num_of_served_tai = mme_cfg.num_of_served_tai;
    memcpy(&self.served_tai, &mme_cfg.served_tai, sizeof(self.served_tai));

    /* enb_s1c_ip */
    memcpy(&self.enb_s1c_ip, &mme_cfg.enb_s1c_ip, sizeof(self.enb_s1c_ip));


    memcpy(&self.cfg_version, &mme_cfg.cfg_ver, sizeof(self.cfg_version));

    snprintf(cmd, sizeof(cmd)-1, "echo %s > /epc/cfg/mme_cfg_ver", self.cfg_version);
    system(cmd);
...
status_t mme_is_enb_s1c_ip_valid(c_sockaddr_t *addr)
{
    int family;

    d_assert(addr, return CORE_ERROR, "");
    family = addr->c_sa_family;

    for(int i=0; i<MAX_NUM_OF_ENB; i++)
    {
        ip_t *ip = &self.enb_s1c_ip[i];
        if (ip->ipv4 == 0 && ip->ipv6 == 0)
        {
            break;
        }
        
        if (ip->ipv4 == 1 && family == AF_INET)
        {
            if (memcmp(&addr->sin.sin_addr, &ip->addr, IPV4_LEN) == 0)
            {
                return CORE_OK;
            }
        }

        if (ip->ipv6 == 1 && family == AF_INET6)
        {
            if (memcmp(&addr->sin6.sin6_addr, &ip->addr6, IPV6_LEN) == 0)
            {
                return CORE_OK;
            }
        }
