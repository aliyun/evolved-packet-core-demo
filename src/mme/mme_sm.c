...
void mme_state_operational(fsm_t *s, event_t *e)
{
    ...
    switch (event_get(e))
    {
        ...
        case MME_EVT_S1AP_MESSAGE:
        {
            ...
            d_assert(FSM_STATE(&enb->sm), pkbuf_free(pkbuf); break, "No S1AP State Machine");

            rv = s1ap_decode_pdu(&message, pkbuf);
            if (rv != CORE_OK)
            {
                d_print_hex(pkbuf->payload, pkbuf->len);
                d_assert(0, s1ap_free_pdu(&message); pkbuf_free(pkbuf); break, "Can't decode S1AP_PDU");
            }
        ...
        case MME_EVT_S1AP_S1_HOLDING_TIMER:
        {
            ...
            d_warn("Implicit S1 release(s1 timer)");
            d_warn("    ENB_UE_S1AP_ID[%d] MME_UE_S1AP_ID[%d]", enb_ue->enb_ue_s1ap_id, enb_ue->mme_ue_s1ap_id);
        ...
        case MME_EVT_EMM_MESSAGE:
        {
            ...
            d_assert(pkbuf, break, "Null param");
            d_assert(nas_emm_decode(&message, pkbuf) == CORE_OK, pkbuf_free(pkbuf); break, "Can't decode NAS_EMM");
            ...
            if (!mme_ue)
            {
                ...
                else
                {
                    ...
                    if (h.integrity_protected)
                    {
                        ...
                        h.ciphered = 0;
                        d_assert(nas_security_decode(mme_ue, h, pkbuf) == CORE_OK,
                            pkbuf_free(pkbuf); return, "nas_security_decode failed");
                    }
                }
                ...
                if (ECM_CONNECTED(mme_ue))
                {
                   ...
                   /* Implcit S1 release */
                    d_trace(5, "Implicit S1 release(ecm2)\n");
            ...
            d_assert(FSM_STATE(&mme_ue->sm), pkbuf_free(pkbuf); break, "No EMM State Machine");
            ...
            if (FSM_CHECK(&mme_ue->sm, emm_state_exception))
            {
                ...
                d_assert(rv == CORE_OK,,"mme_send_delete_session_or_ue_context_release() failed");
            }
        ...
        case MME_EVT_ESM_MESSAGE:
        {
            ...
            pkbuf = (pkbuf_t *)event_get_param2(e);
            d_assert(pkbuf, break, "Null param");
            d_assert(nas_esm_decode(&message, pkbuf) == CORE_OK, pkbuf_free(pkbuf); break, "Can't decode NAS_ESM");
        ...
        case MME_EVT_S6A_MESSAGE:
        {
            ...
            if (s6a_message->result_code != ER_DIAMETER_SUCCESS)
            {
                ...
                rv = nas_send_attach_reject(mme_ue, EMM_CAUSE_IMSI_UNKNOWN_IN_HSS, ESM_CAUSE_PROTOCOL_ERROR_UNSPECIFIED);
                d_assert(rv == CORE_OK,,"nas_send_attach_reject failed");
            ...
            switch(s6a_message->cmd_code)
            {
                ...
                case S6A_CMD_CODE_UPDATE_LOCATION:
                {
                    ...
                    else if (FSM_CHECK(&mme_ue->sm, emm_state_registered))
                    {
                        ...
                        {
                            rv = nas_send_tau_accept(mme_ue, S1AP_ProcedureCode_id_InitialContextSetup);
                        ...
                        else if (mme_ue->nas_eps.type == MME_EPS_TYPE_SERVICE_REQUEST)
                        {
                            ...
                            d_assert(rv == CORE_OK,,"s1ap_send_initial_context_setup_request() failed");
                        }
                        else
                            d_assert(0,, "Invalid EPS-Type[%d]", mme_ue->nas_eps.type);
                    }
                    else
                        d_assert(0,, "Invaild EMM state for EPS-Type[%d]", mme_ue->nas_eps.type);
                    break;
                }
                default:
                {
        ...
        case MME_EVT_S11_MESSAGE:
        {
            ...
            d_assert(mme_ue, pkbuf_free(pkbuf); break, "No UE Context(TEID:%d)", message.h.teid);
