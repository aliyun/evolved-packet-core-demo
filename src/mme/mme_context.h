...
#define SOFT_SERIAL_LEN             32
#define SOFT_KEY_LEN                64
#define S1_INTF_LEN                 32
    
#define	MME_CNT_MEM_FILE            "/epc/log/epc_cnts_file"
#define MAX_LEN_OF_VERSION          32

typedef enum {
    CNT_S1AP_IN_S1SetupRequest=0,
    CNT_S1AP_IN_initialUEMessage,
    CNT_S1AP_IN_uplinkNASTransport,
    CNT_S1AP_IN_UECapabilityInfoIndication,
    CNT_S1AP_IN_UEContextReleaseRequest,
    CNT_S1AP_IN_PathSwitchRequest,
    CNT_S1AP_IN_eNBConfigurationTransfer,
    CNT_S1AP_IN_HandoverRequired,
    CNT_S1AP_IN_HandoverCancel,
    CNT_S1AP_IN_eNBStatusTransfer,
    CNT_S1AP_IN_HandoverNotification,
    CNT_S1AP_IN_Reset,
    CNT_S1AP_IN_E_RABModificationIndication,
    CNT_S1AP_IN_InitialContextSetupResponse,
    CNT_S1AP_IN_E_RABSetupResponse,
    CNT_S1AP_IN_E_RABModifyResponse,
    CNT_S1AP_IN_E_RABReleaseResponse,
    CNT_S1AP_IN_UEContextReleaseComplete,
    CNT_S1AP_IN_HandoverRequestAck,
    CNT_S1AP_IN_WriteReplaceWarningResponse,
    CNT_S1AP_IN_KillResponse,
    CNT_S1AP_IN_InitialContextSetupFailure,
    CNT_S1AP_IN_HandoverFailure,
    CNT_S1AP_IN_MAX
} cnt_s1ap_in_e;

typedef enum {
    CNT_S1AP_OUT_S1SetupResponse=0,
    CNT_S1AP_OUT_S1SetupFailure,
    CNT_S1AP_OUT_DownlinkNasTransport,
    CNT_S1AP_OUT_InitialContextSetupRequest,
    CNT_S1AP_OUT_ERabSetupRequest,
    CNT_S1AP_OUT_ERabModifyRequest,
    CNT_S1AP_OUT_ERabReleaseCommand,
    CNT_S1AP_OUT_UeContextReleaseCommand,
    CNT_S1AP_OUT_Paging,
    CNT_S1AP_OUT_MmeConfigurationTransfer,
    CNT_S1AP_OUT_PathSwitchAck,
    CNT_S1AP_OUT_PathSwitchFailure,
    CNT_S1AP_OUT_ERabModificationConfirm,
    CNT_S1AP_OUT_HandoverCommand,
    CNT_S1AP_OUT_HandoverPreparationFailure,
    CNT_S1AP_OUT_HandoverRequest,
    CNT_S1AP_OUT_HandoverCancelAck,
    CNT_S1AP_OUT_MmeStatusTransfer,
    CNT_S1AP_OUT_ErrorIndication,
    CNT_S1AP_OUT_S1Reset,
    CNT_S1AP_OUT_S1ResetPartial,
    CNT_S1AP_OUT_S1ResetAck,
    CNT_S1AP_OUT_WriteReplaceWarningRequest,
    CNT_S1AP_OUT_KillRequest,
    CNT_S1AP_OUT_MAX
} cnt_s1ap_out_e;

typedef enum {
    CNT_NAS_IN_ServiceRequest=0,
    CNT_NAS_IN_AttachRequest,
    CNT_NAS_IN_AttachComplete,
    CNT_NAS_IN_IdentityResponse,
    CNT_NAS_IN_TauRequest,
    CNT_NAS_IN_TauComplete,
    CNT_NAS_IN_EmmStatus,
    CNT_NAS_IN_DetachRequest,
    CNT_NAS_IN_AuthenticationResponse,
    CNT_NAS_IN_AuthenticationFailure,
    CNT_NAS_IN_SecurityModeComplete,
    CNT_NAS_IN_SecurityModeReject,
    CNT_NAS_IN_PdnConnectivityRequest,
    CNT_NAS_IN_InformationResponse,
    CNT_NAS_IN_ActivateDefaultEpsBearerContextAccept,
    CNT_NAS_IN_ActivateDedicatedEpsBearerContextAccept,
    CNT_NAS_IN_PdnDisconnectRequest,
    CNT_NAS_IN_ModifyEpsBearerContextAccept,
    CNT_NAS_IN_DeactivateEpsBearerContextAccept,
    CNT_NAS_IN_MAX
} cnt_nas_in_e;

typedef enum {
    CNT_NAS_OUT_AttachAccept=0,
    CNT_NAS_OUT_AttachReject,
    CNT_NAS_OUT_IdentityRequest,
    CNT_NAS_OUT_AuthenticationRequest,
    CNT_NAS_OUT_AuthenticationReject,
    CNT_NAS_OUT_SecurityModeCommand,
    CNT_NAS_OUT_DetachAccept,
    CNT_NAS_OUT_TauAccept,
    CNT_NAS_OUT_TauReject,
    CNT_NAS_OUT_ServiceReject,
    CNT_NAS_OUT_PdnConnectivityReject,
    CNT_NAS_OUT_InformationRequest,
    CNT_NAS_OUT_ActivateDefaultBearerContextRequest,
    CNT_NAS_OUT_ActivateDedicatedBearerContextRequest,
    CNT_NAS_OUT_ModifyBearerContextRequest,
    CNT_NAS_OUT_DeactivateBearerContextRequest,
    CNT_NAS_OUT_MAX
} cnt_nas_out_e;

typedef struct {
    c_uint32_t      s1ap_in_cnt[CNT_S1AP_IN_MAX];
    c_uint32_t      s1ap_out_cnt[CNT_S1AP_IN_MAX];
    c_uint32_t      nas_in_cnt[CNT_NAS_IN_MAX];
    c_uint32_t      nas_out_cnt[CNT_NAS_OUT_MAX];
} mme_cnt_ctx_t;

typedef struct _mme_cfg_t mme_cfg_t;
...
typedef struct _mme_context_t {
    ...
    char            serial[SOFT_SERIAL_LEN];
    char            key[SOFT_KEY_LEN];
    char            s1_intf[S1_INTF_LEN];

    mme_cnt_ctx_t   *mme_cnt_ctx;
    
    c_int8_t        cfg_version[MAX_LEN_OF_VERSION];
    ip_t            enb_s1c_ip[MAX_NUM_OF_ENB];
} mme_context_t;

typedef struct _mme_cfg_t {
    c_int8_t        cfg_ver[MAX_LEN_OF_VERSION];
    c_uint16_t      num_of_served_tai;
    struct {
        tai0_list_t list0;
        tai2_list_t list2;
    } served_tai[MAX_NUM_OF_SERVED_TAI];
...
#define GTP_COUNTER_MODIFY_BEARER_BY_E_RAB_MODIFY               3
    struct {
        c_uint8_t request;
        c_uint8_t response;
...
typedef struct _s1u_bind_by_cell_id_t {
    ip_t        s1u_ip;
    c_uint32_t  cells[MAX_NUM_OF_BIND_CELL];
} s1u_bind_by_cell_id_t;
...
CORE_DECLARE(mme_sgw_t*)    mme_sgw_add(c_sockaddr_t *all_list, int no_ipv4, int no_ipv6, int prefer_ipv4);
CORE_DECLARE(status_t )     mme_sgw_remove(mme_sgw_t *sgw);
CORE_DECLARE(status_t )     mme_sgw_remove_all();

CORE_DECLARE(mme_pgw_t*)    mme_pgw_add(c_sockaddr_t *all_list, int no_ipv4, int no_ipv6, int prefer_ipv4);
...
CORE_DECLARE(status_t)      mme_enb_set_enb_id(mme_enb_t *enb, c_uint32_t enb_id);
...
CORE_DECLARE(status_t)      enb_ue_switch_to_enb(enb_ue_t *enb_ue, mme_enb_t *new_enb);
CORE_DECLARE(enb_ue_t*)     enb_ue_find(index_t index);
CORE_DECLARE(enb_ue_t*)     enb_ue_find_by_enb_ue_s1ap_id(mme_enb_t *enb, c_uint32_t enb_ue_s1ap_id);
...
CORE_DECLARE(status_t)      mme_ue_set_imsi(mme_ue_t *mme_ue, c_int8_t *imsi_bcd);
 ...
 *   - Delete Indirect Data Forwarding Tunnel Request/Response
 */
CORE_DECLARE(status_t)      mme_ue_associate_enb_ue(mme_ue_t *mme_ue, enb_ue_t *enb_ue);
CORE_DECLARE(status_t)      enb_ue_deassociate(enb_ue_t *enb_ue);
CORE_DECLARE(status_t)      mme_ue_deassociate(mme_ue_t *mme_ue);
CORE_DECLARE(status_t)      source_ue_associate_target_ue(enb_ue_t *source_ue, enb_ue_t *target_ue);
CORE_DECLARE(status_t)      source_ue_deassociate_target_ue(enb_ue_t *enb_ue);
CORE_DECLARE(status_t)      mme_remove_source_ue_when_ho_success(enb_ue_t *source_ue, enb_ue_t *target_ue);
CORE_DECLARE(void)          mme_restore_info_when_ho_failure(enb_ue_t *source_ue, enb_ue_t *target_ue);
...
CORE_DECLARE(mme_sess_t*)   mme_sess_find_by_pti(mme_ue_t *mme_ue, c_uint8_t pti);
CORE_DECLARE(mme_sess_t*)   mme_sess_find_by_ebi(mme_ue_t *mme_ue, c_uint8_t ebi);
CORE_DECLARE(mme_sess_t*)   mme_sess_find_by_apn(mme_ue_t *mme_ue, c_int8_t *apn);
...
CORE_DECLARE(mme_bearer_t*) mme_bearer_find_by_sess_ebi(mme_sess_t *sess, c_uint8_t ebi);
CORE_DECLARE(mme_bearer_t*) mme_bearer_find_by_ue_ebi(mme_ue_t *mme_ue, c_uint8_t ebi);
CORE_DECLARE(mme_bearer_t*) mme_bearer_find_or_add_by_message(mme_ue_t *mme_ue, nas_message_t *message);
...
CORE_DECLARE(pdn_t*)        mme_pdn_find_by_apn(mme_ue_t *mme_ue, c_int8_t *apn);
...
CORE_DECLARE(ip_t *)        mme_get_s1u_ip_by_cell_id(c_uint32_t cell_id);
CORE_DECLARE(status_t)      mme_add_s1u_ip_bind_cell_id_entry(const char *ip, c_uint32_t *cells);

CORE_DECLARE(mme_cnt_ctx_t*)        mme_cnt_memory_init();
CORE_DECLARE(void)          mme_inc_cnt_s1ap_in(cnt_s1ap_in_e type);
CORE_DECLARE(void)          mme_inc_cnt_s1ap_out(cnt_s1ap_out_e type);
CORE_DECLARE(void)          mme_inc_cnt_nas_in(cnt_nas_in_e type);
CORE_DECLARE(void)          mme_inc_cnt_nas_out(cnt_nas_out_e type);

CORE_DECLARE(status_t)      mme_is_enb_s1c_ip_valid(c_sockaddr_t *addr);
CORE_DECLARE(status_t)      mme_check_and_apply_dynamic_cfg();
