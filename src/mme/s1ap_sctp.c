...
status_t s1ap_send(sock_id sock, pkbuf_t *pkbuf, c_sockaddr_t *addr, c_uint16_t stream_no)
{
    int sent;
    ...
    sent = core_sctp_sendmsg(sock, pkbuf->payload, pkbuf->len, addr, SCTP_S1AP_PPID, stream_no);
...
#if 0
        if (mme_is_enb_s1c_ip_valid(addr) == CORE_ERROR)
        {
            
            d_trace(0, "eNB-S1 reject[%s]\n", CORE_ADDR(addr, buf));
            CORE_FREE(addr);
...
int s1ap_recv_handler(sock_id sock, void *data)
{
    ...
    size = core_sctp_recvmsg(sock, pkbuf->payload, pkbuf->len, NULL, &sinfo, &flags);
    if (size < 0)
    {
        d_error("core_sctp_recvmsg(%d) failed(%d:%s)", size, errno, strerror(errno));
        return size;
    }

    if (flags & MSG_NOTIFICATION)
    {
        union sctp_notification *not = (union sctp_notification *)pkbuf->payload;

        switch(not->sn_header.sn_type) 
        {
            case SCTP_ASSOC_CHANGE :
            {
                d_trace(5, "SCTP_ASSOC_CHANGE:[T:%d, F:0x%x, S:%d, I/O:%d/%d]\n", 
                ...
                if (not->sn_assoc_change.sac_state == SCTP_COMM_UP)
                {
                    ...
                    event_set_param2(&e, (c_uintptr_t)addr);
                    event_set_param3(&e, (c_uintptr_t)not->sn_assoc_change.sac_inbound_streams);
                    event_set_param4(&e, (c_uintptr_t)not->sn_assoc_change.sac_outbound_streams);
            ...
            default :
            {
                d_error("Discarding event with unknown flags:0x%x type:0x%x", flags, not->sn_header.sn_type);
                break;
            }
        }
    }
    else if (flags & MSG_EOR)
    {
        ...
        {
            //d_error("Event MME_EVT_S1AP_MESSAGE");
        }
    ...
    {
        d_assert(0, pkbuf_free(pkbuf); return -1, "Unknown flags : 0x%x", flags);
    }
