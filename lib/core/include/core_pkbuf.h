 ...
 */
#define PKBUG_DBG_TRACE 1
#ifdef PKBUG_DBG_TRACE
#define MAX_SIZEOF_DBG_TRACE 128
#define MAX_SIZEOF_HEADROOM (128+MAX_SIZEOF_DBG_TRACE)
CORE_DECLARE(pkbuf_t *) pkbuf_alloc_dbg(c_uint16_t headroom, c_uint16_t length, const char* func, int line);

#define pkbuf_alloc(A,B) pkbuf_alloc_dbg(A+MAX_SIZEOF_DBG_TRACE, B, __FILE__, __LINE__)
#else
...
CORE_DECLARE(void) pkbuf_dump(pkbuf_t *pkbuf);
/**
...
#ifdef PKBUG_DBG_TRACE
CORE_DECLARE(void *) core_malloc_dbg(size_t size, const char* func, int line);
#define core_malloc(size) core_malloc_dbg(size, __FILE__, __LINE__)

CORE_DECLARE(void *) core_calloc_dbg(size_t nmemb, size_t size, const char* func, int line);
#define core_calloc(nmemb, size) core_calloc_dbg(nmemb, size, __FILE__, __LINE__)

CORE_DECLARE(void)  show_unfreeed_bufs(void);
#else
