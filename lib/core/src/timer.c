...
void tm_service_init(tm_service_t *tm_service, expire_func_t expire_func)
{
    memset(tm_service, 0x00, sizeof(tm_service_t));
    list_init(&tm_service->active_list);
    list_init(&tm_service->idle_list);
    if (expire_func) {
        tm_service->expire_func = expire_func;
    }
    return;
}
...
status_t tm_execute_tm_service(tm_service_t *p_tm_s, c_uintptr_t data)
{
    ...
    while(tm)
    {
        if(tm->expire_time < cur_time)
        {
            ...
            if (CORE_OK == tm->expire_func(data, tm->param1, tm->param2, tm->param3,
                    tm->param4, tm->param5, tm->param6) || tm->ex_repeat > 5) {
...
{
    c_time_t cur_time = time_as_msec(time_now());
    ...
    tm->expire_time = cur_time + (c_time_t)tm->duration;
