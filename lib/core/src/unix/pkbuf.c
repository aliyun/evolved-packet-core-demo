...
#define MAX_NUM_OF_CLBUF (256*100)
#define MAX_NUM_OF_PKBUF (256*100)
...
#define MAX_NUM_OF_CLUSTER_128      (256*100)
#define MAX_NUM_OF_CLUSTER_256      (256*100) 
#define MAX_NUM_OF_CLUSTER_512      (256*100)
#define MAX_NUM_OF_CLUSTER_1024     (256*100)
#define MAX_NUM_OF_CLUSTER_2048     (256*10)
#define MAX_NUM_OF_CLUSTER_8192     (256*10)
...
#ifdef PKBUG_DBG_TRACE
#if 0
static void dump_hex(unsigned char *ptr ,int len)
{
    printf("\t\t\t\t\t");
    for(int i=0; i<len; i++) {
        printf("%02x ", *(ptr+i));
...
if (strcmp(cluster, "1747 OCTET_STRING.c") == 0) { \
    dump_hex((unsigned char *)(cluster+128), 64); \
}
#endif

#define pool_dump_using_node(__pname) { \
    int i=0, j=0, is_free=0; \
    d_trace(0, "size=%d avail=%d h=%d t=%d\n", (__pname)->size, (__pname)->avail, (__pname)->head, (__pname)->tail); \
    d_trace(0, "not free:\n"); \
    for(i=0; i<(__pname)->size; i++) { \
        is_free = 0; \
        for(j=0; j<(__pname)->avail; j++) { \
            if ((__pname)->free[((__pname)->head+j)%(__pname)->size] == &((__pname)->pool[i])) { \
                is_free = 1; \
                break; \
            } \
        } \
        if (is_free == 0) { \
            char* cluster = (char*)(&((__pname)->pool[i])); \
            d_assert(cluster, continue, "cluster is NULL"); \
            d_trace(0, "\t%p\t%s\n", cluster, cluster); \
        } \
    } \
}
...
void pkbuf_show(void)
{
    if (pool_used(&pkbuf_pool))
        d_error("%d not freed in pkbuf_pool[%d]", pool_used(&pkbuf_pool), pool_size(&pkbuf_pool));
    d_trace(9, "%d not freed in pkbuf_pool[%d]\n", pool_used(&pkbuf_pool), pool_size(&pkbuf_pool));

    if (pool_used(&clbuf_pool))
        d_error("%d not freed in clbuf_pool[%d]", pool_used(&clbuf_pool), pool_size(&clbuf_pool));
    d_trace(9, "%d not freed in clbuf_pool[%d]\n", pool_used(&clbuf_pool), pool_size(&clbuf_pool));

    if (pool_used(&cluster_128_pool))
        d_error("%d not freed in cluster128_pool[%d]", pool_used(&cluster_128_pool), pool_size(&cluster_128_pool));
    ...
    if (pool_used(&cluster_256_pool))
        d_error("%d not freed in cluster256_pool[%d]", pool_used(&cluster_256_pool), pool_size(&cluster_256_pool));
    ...
    if (pool_used(&cluster_512_pool))
        d_error("%d not freed in cluster512_pool[%d]", pool_used(&cluster_512_pool), pool_size(&cluster_512_pool));
    ...
    if (pool_used(&cluster_1024_pool))
        d_error("%d not freed in cluster1024_pool[%d]", pool_used(&cluster_1024_pool), pool_size(&cluster_1024_pool));
    ...
    if (pool_used(&cluster_2048_pool))
        d_error("%d not freed in cluster2048_pool[%d]", pool_used(&cluster_2048_pool), pool_size(&cluster_2048_pool));
    ...
    if (pool_used(&cluster_8192_pool))
        d_error("%d not freed in cluster8192_pool[%d]", pool_used(&cluster_8192_pool), pool_size(&cluster_8192_pool));
...
static clbuf_t* clbuf_alloc(c_uint16_t length)
{
    ...
    d_assert(clbuf, show_unfreeed_bufs();return NULL, "No more free clbuf. ");
    ...
    d_assert(cluster, pool_free_node(&clbuf_pool, clbuf); show_unfreeed_bufs();return NULL, "No more free cluster. length:%d requested", length);
...
#ifdef PKBUG_DBG_TRACE
pkbuf_t* pkbuf_alloc_real(c_uint16_t headroom, c_uint16_t length)
#else
...
#endif
{
    ...
    d_assert(clbuf, util_trigger_device_reboot();return NULL, "Can't allocate clbuf(length:%d)", length);

    pool_alloc_node(&pkbuf_pool, &np);
    d_assert(np, clbuf_free(clbuf); util_trigger_device_reboot();return NULL, "No more free pkbuf");
    ret = np;
...
#ifdef PKBUG_DBG_TRACE
CORE_DECLARE(pkbuf_t *) pkbuf_alloc_dbg(c_uint16_t headroom, c_uint16_t length, const char* func, int line)
{
    pkbuf_t* pkbuf = NULL;
    clbuf_t *clbuf;

    pkbuf = pkbuf_alloc_real(headroom, length);

    d_assert(pkbuf, return pkbuf, "pkbuf alloc failed!");
    

    clbuf = pkbuf->clbuf;
    d_assert(pkbuf->clbuf, return pkbuf, "pkbuf has no clbuf");
    d_assert(pkbuf->clbuf->cluster, return pkbuf, "clbuf has no cluster");

    snprintf(clbuf->cluster, MAX_SIZEOF_DBG_TRACE, "%d %s", line, func);
    //printf("\t%s\n", (char *)clbuf->cluster);
...
void pkbuf_dump(pkbuf_t *pkbuf)
{
    int i=0;
    c_uint8_t *ptr = NULL;

    d_assert(pkbuf, return, "NULL");
    d_assert(pkbuf->payload, return, "NULL");

    ptr = (c_uint8_t *)pkbuf->payload;
...
#ifdef PKBUG_DBG_TRACE
void *core_malloc_dbg(size_t size, const char *file, int line)
{
    ...
    p = pkbuf_alloc_dbg(headroom+MAX_SIZEOF_DBG_TRACE, size, file, line);
    d_assert(p, return NULL, "pkbuf_alloc failed(headroom:%d, size:%d)", headroom, size);
    d_assert(p->next == NULL, pkbuf_free(p);return NULL, "core_malloc should not be fragmented");
...
#ifdef PKBUG_DBG_TRACE
void *core_calloc_dbg(size_t nmemb, size_t size, const char *file, int line)
{
    void *ptr = NULL;

    ptr = core_malloc_dbg(nmemb * size, file, line);
...
void show_unfreeed_bufs()
{
    d_trace(0, "128===>");
    pool_dump_using_node(&cluster_128_pool);
    d_trace(0, "256===>");
    pool_dump_using_node(&cluster_256_pool);
    d_trace(0, "512===>");
    pool_dump_using_node(&cluster_512_pool);
    d_trace(0, "1024===>");
    pool_dump_using_node(&cluster_1024_pool);
    d_trace(0, "2048===>");
    pool_dump_using_node(&cluster_2048_pool);
    d_trace(0, "8192===>");
    pool_dump_using_node(&cluster_8192_pool);
}
#else
