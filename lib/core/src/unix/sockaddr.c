...
status_t core_addaddrinfo(c_sockaddr_t **sa_list, int family, const char *hostname, c_uint16_t port, int flags)
{
    int rc;
    ...
    if (rc != 0)
    {
        d_error("getaddrinfo(%d:%s:%d:0x%x) failed(%d:%s)", family, hostname, port, flags, errno, strerror(errno));
        return CORE_ERROR;
    }
    ...
    if (prev == NULL)
    {
        d_error("core_getaddrinfo(%d:%s:%d:%d) failed(%d:%s)", family, hostname, port, flags, errno, strerror(errno));
        return CORE_ERROR;
    }
...
const char *core_inet_ntop(void *sa, char *buf, int buflen)
{
    ...
    switch(family)
    {
        case AF_INET:
            ...
            return inet_ntop(family, &sockaddr->sin.sin_addr, buf, INET_ADDRSTRLEN);
        case AF_INET6:
            d_assert(buflen >= CORE_ADDRSTRLEN, return NULL,);
            return inet_ntop(family, &sockaddr->sin6.sin6_addr, buf, INET6_ADDRSTRLEN);
        default:
...
status_t core_inet_pton(int family, const char *src, void *sa)
{
    ...
    {
        case AF_INET:
            return inet_pton(family, src, &dst->sin.sin_addr) == 1 ? CORE_OK : CORE_ERROR;
        case AF_INET6:
            return inet_pton(family, src, &dst->sin6.sin6_addr) == 1 ? CORE_OK : CORE_ERROR;
        default:
...
int sockaddr_is_equal(void *p, void *q)
{
    ...
    if (a->c_sa_family == AF_INET && memcmp(&a->sin.sin_addr, &b->sin.sin_addr, sizeof(struct in_addr)) == 0)
        return 1;
    else if (a->c_sa_family == AF_INET6 && memcmp(&a->sin6.sin6_addr, &b->sin6.sin6_addr, sizeof(struct in6_addr)) == 0)
        return 1;
    else
 ...
 */
static status_t parse_ip(ipsubnet_t *ipsub, const char *ipstr, int network_allowed)
{
