...
status_t sock_close(sock_id id)
{
    sock_t *sock = (sock_t *)id;
    d_assert(id, return CORE_ERROR,);
...
status_t sock_setsockopt(sock_id id, c_int32_t opt, c_int32_t on)
{
    ...
    switch(opt)
    {
        case SOCK_O_REUSEADDR:
            ...
            {
                if (setsockopt(sock->fd, SOL_SOCKET, SO_REUSEADDR, (void *)&one, sizeof(int)) == -1)
                {
                    return errno;
                }
...
status_t sock_connect(sock_id id, c_sockaddr_t *addr)
{
    ...
    {
        d_error("socket connect[%s]:%d failed(%d:%s)", CORE_ADDR(addr, buf), CORE_PORT(addr), errno, strerror(errno));
        return CORE_ERROR;
    }
...
status_t sock_set_bind_cell(sock_node_t *node, sock_bind_cell_t *bind_cell)
{
    d_assert(node, return CORE_ERROR,);
    d_assert(bind_cell, return CORE_ERROR,);

    node->ext_data = core_calloc(1, sizeof(sock_bind_cell_t));
    d_assert(node->ext_data, return CORE_ERROR, "alloc memory for bind_cell failed!");

    node->ext_type = SOCK_EXT_BIND_CELL;
    memcpy(node->ext_data, bind_cell, sizeof(sock_bind_cell_t));
...
status_t sock_fill_scope_id_in_local(c_sockaddr_t *sa_list)
{
    ...
    for (addr = sa_list; addr != NULL; addr = addr->next)
    {
        ...
        for (cur = iflist; cur != NULL; cur = cur->ifa_next)
        {
            ...
            if (memcmp(&addr->sin6.sin6_addr, &ifaddr->sin6.sin6_addr, sizeof(struct in6_addr)) == 0)
            {
...
ssize_t sock_write(sock_id id, const void *buf, size_t len)
{
    ...
    if (size < 0)
    {
        d_error("sock_write(len:%ld) failed(%d:%s)", len, errno, strerror(errno));
    }
...
ssize_t sock_read(sock_id id, void *buf, size_t len)
{
    ...
    if (size < 0)
    {
        d_error("sock_read(len:%ld) failed(%d:%s)", len, errno, strerror(errno));
    }
...
ssize_t core_send(sock_id id, const void *buf, size_t len, int flags)
{
    ...
    if (size < 0)
    {
        d_error("core_send(len:%ld) failed(%d:%s)", len, errno, strerror(errno));
    }

    return size;
}

ssize_t core_sendto(sock_id id, const void *buf, size_t len, int flags, const c_sockaddr_t *to)
{
    sock_t *sock = (sock_t *)id;
    ssize_t size;
    socklen_t addrlen;
    ...
    if (size < 0)
    {
        d_error("core_sendto(len:%ld) failed(%d:%s)", len, errno, strerror(errno));
    }
...
ssize_t core_recv(sock_id id, void *buf, size_t len, int flags)
{
    ...
    if (size < 0)
    {
        d_error("core_recv(len:%ld) failed(%d:%s)", len, errno, strerror(errno));
    }

    return size;
}

ssize_t core_recvfrom(sock_id id, void *buf, size_t len, int flags, c_sockaddr_t *from)
{
    sock_t *sock = (sock_t *)id;
    ssize_t size;
    ...
    if (size < 0)
    {
        d_error("corek_recvfrom(len:%ld) failed(%d:%s)", len, errno, strerror(errno));
    }
