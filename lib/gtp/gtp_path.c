...
c_sockaddr_t *gtp_local_addr_by_cell_id(list_t *list, c_uint32_t cell_id)
{
    sock_node_t *snode = NULL;
    c_sockaddr_t *addr = NULL;
    c_uint32_t *cell = NULL;
    c_uint32_t i = 0;
    ...
    for (snode = list_first(list); snode; snode = list_next(snode))
    {
        ...
        if (snode->ext_type == SOCK_EXT_BIND_CELL) {
            cell = (c_uint32_t *)snode->ext_data;
            for (i=0; i<MAX_NUM_OF_BIND_CELL; i++) {
                if (cell_id == *(cell+i)) {
                    found = 1;
                    break;
                }
...
int gtp_get_local_port(gtp_node_t *gnode)
{
    return udp_client_get_local_port(gnode->sock);
}
