...
#define SIZE_OF_GTP_XACT_POOL       (64*16)
...
status_t gtp_xact_init(tm_service_t *tm_service, c_uintptr_t response_event, c_uintptr_t holding_event)
{
    d_assert(gtp_xact_initialized == 0, return CORE_ERROR, "GTP Transaction already has been initialized");
        ...
        gtp_node_t *gnode, gtp_header_t *hdesc, pkbuf_t *pkbuf)
{
    ...
    if (g_response_event)
    {
        xact->tm_response = timer_create(g_tm_service, g_response_event, GTP_T3_RESPONSE_DURATION);
...
status_t gtp_set_pkt_hdr(gtp_header_t *hdesc, pkbuf_t *pkbuf)
{
    gtp_header_t *h = NULL;
